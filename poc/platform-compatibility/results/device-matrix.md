...
- **처리 상태:** 스펙 003 **로컬 구현 완료**(`f581242`와 분리 커밋). `.canvas-wrap` 폭을 `min(100%,45vh)`+`aspect-ratio:3/4`로 단일화(`max-height:60vh` 제거) + 전체 viewport E2E 비율 회귀 추가. 자동검증 typecheck 0/unit 30/build/e2e 11(각 viewport `abs(w/h-0.75)<=0.01`) 통과. E2E landscape 측정: 844×390 css 175.5×234, 932×430 css 193.5×258, 1024×768 css 345.6×460.8 — 전부 ratio 0.75. **남은 것 = 사용자 실기기 4환경 세로/가로 회전 재검증** → 아래 게이트 행 갱신. 4환경 PASS 전까지 판정 **FAIL 유지**, 001 종료 보류(스펙 003 DONE §5).
