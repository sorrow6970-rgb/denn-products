...
### 2. 레이아웃 브라우저별 편차 (미해결, 우선)
- 증상: 카카오 인앱=액자 작음 / 삼성브라우저 가로=너무 큼 / 카카오=주소창 안 사라짐.
- 원인 가설: `lvh`가 인앱웹뷰(카카오)·삼성 가로에서 뷰포트 계산이 브라우저마다 달라 `applyStableScale`의 `area.clientHeight` 기준이 흔들림. lvh 단일 해법이 전 브라우저 커버 못 함.
- 접근: 실기기별 `?debug=1`로 각 브라우저의 clientHeight/스케일 실측 후, 뷰포트 단위 대신 **JS로 가시영역 측정(visualViewport API)해 스케일** 등 재설계 검토. [[feedback-measure-dom-before-iterating]]
