...
### 6. 새로고침 시 UI/UX 잔상 깜빡임(튐) — 프레임툴·룸뷰 공통
- 증상: **새로고침할 때마다 예전 UI/UX 잔상이 빠르게 깜빡**(레이아웃/액자가 순간 다른 크기·위치로 보였다가 정착). 어제 07-03 핸드오프의 "리로드 깜빡임"과 겹치는 이슈.
- 원인(추정): 데이터(ADM)·스케일 확정 **전에 기본/이전 레이아웃이 먼저 렌더**됐다가, 발행본 로드+스케일 적용 시 재조정되며 번쩍임. 스케일러가 `renderFrame`/resize 후 `transition:none`으로 즉시 재적용(L10216)하는 것도 순간 점프 유발.
- 해결방향: **첫 정상 fit 전까지 프레임/룸 콘텐츠 숨김(opacity 0→1 페이드)** = 로딩 커버. 프레임 wrap엔 이미 `visibility:hidden→ready` 가드(L10191/10229)가 있으나 **페이지 전체(헤더/메뉴/룸) 잔상**은 안 덮음 → 부팅 완료(init+첫 스케일) 시점까지 전체 페이드 커버 추가 검토.

### 8. (낮음) 기타
- 카카오 프레임 작음(웹뷰 높이 731로 실제 작음, 하단여백 축소로 일부 완화). 룸 캔버스 색 반전(일시적, 새로고침 사라짐 — 재발 시 스샷).
