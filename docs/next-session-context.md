...
- **[UX] 템플릿이 사이즈 종속이라 초기 화면에 시안 종류가 적게 노출** — `buildFrameTplGrid`가 `templateMatchesCurrentFrameSize`로 1차 필터(템플릿 사이즈 태그 ↔ 현재 선택 사이즈 일치 시만 노출). 초기부터 다양한 시안을 보여주려면 "사이즈 미선택 시 전체 노출 + 템플릿 선택 시 사이즈 자동 적용"(A안) 등 로직 변경 필요. 재준 결정 대기.
