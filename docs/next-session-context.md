...
- 케이스 기능 개발 끝나기 전까지 사용자가 폰케이스 탭에 진입하지 못하도록 막아둠.
- **[재준 결정 필요]** 차단 방식:
  - A. 탭 자체 숨김 (목록에서 비노출)
  - B. 탭은 보이되 비활성 + "준비 중" 안내
  - C. 클릭 시 토스트로 "오픈 예정" 안내
- 개발 완료 후 한 줄 토글로 다시 열 수 있게 플래그 방식으로 구현.

## [작업 5] 내공간보기 — 패스워드 개별 설정 탭
- 고객 "내공간보기"를 토큰 + 패스워드 방식으로 개별 접근 가능하게.
- 즉 시안별/고객별로 패스워드를 따로 설정해 그 사람만 자기 공간을 열람·설정.
...
## [작업 7] 모바일 하단 탭 UI 수정 + 모바일 최적화
- 모바일 하단 탭바 UI 정리 + 모바일 전용 최적화.
- 목표 방향(이전 세션 확정): 상단 미리보기 풀화면 + 하단 고정 탭바 + 탭 시 바텀시트 슬라이드업.
- **[재준 결정 필요]** 레이아웃: A. PC 세로 단순 재배치 / B. 바텀시트 방식 / C. 기타
