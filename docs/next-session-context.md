...
- 액자(#page-frame) PC 4컬럼 셸(아이콘 레일 | 템플릿 드로어 | 캔버스 | 우측 요소·액션) 구현·push 완료(HEAD `aebf92c`).
...
### 작업 3 진행 중 발견 — 백로그 (별도 처리)
- **[버그] 가로모드(landscape) 이미지 회전 이상** — 가로 비율 이미지가 캔버스에서 90° 돌아가는 등 이상 표시. **UI 아닌 렌더/이미지 방향(EXIF orientation 추정) 처리 문제로 접근.** renderFrame 보호영역 인접 가능성 있어 신중히. (2026-05-28 재준 보고, 별도 작업으로 분리)
- **[UX] 초기 화면 템플릿 다양성** — `buildFrameTplGrid`가 `templateMatchesCurrentFrameSize`로 사이즈 일치 시만 노출 → 초기엔 종류 적음.
  - **중요 제약(재준 2026-05-28)**: 같은 디자인(예: WEDDING)이라도 **사이즈마다 템플릿 이미지가 다름** → 템플릿 = (디자인 × 사이즈) 단위. **제작 제약: 선택 템플릿의 사이즈 = 주문 사이즈여야만 제작 가능**(예: A2 주문에 B5 템플릿 = 제작 불가). 따라서 현재 사이즈 필터는 **"제작 가능 조합만 고르게 하는 안전장치"** → 사이즈-first 반드시 유지. 템플릿-first/전체 자유노출은 **틀린 방향**.
  - **해결 방향**: (1) 선택 사이즈 내 디자인을 다 노출(제작 가능한 것만) — 다양성은 그 사이즈의 디자인 수에 의존. (2) 더 풍부하게 보이려면 **별도 "둘러보기 쇼케이스"**(사이즈 무관 구경용, 실제 선택은 사이즈 확정 후 해당 변형 로드)를 분리 신설. (1)이 기본, (2)는 선택적 기능. 어느 쪽이든 데이터에 디자인↔사이즈변형 연결 키 확인 선행. 별도 단계로 진행.
