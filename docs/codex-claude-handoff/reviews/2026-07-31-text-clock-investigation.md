...
### 3.5 ★ 인쇄/export에 시계가 없다 (CONFIRMED — 결함)

`renderFramePrintV365`(`:11404-11446`)와 구버전 `renderFramePrint`(`:9732` IIFE 내부) **모두
`drawClockLayer` 호출이 0회**다(`awk`로 본문 전 범위 확인). 미리보기 경로에만 존재한다
(`:1802`, `:1812`, `:2596`, `:2607`, `:3131`).

→ **고객은 시계가 있는 화면을 보고 주문하지만 인쇄 파일에는 시계가 없다.**
이것이 스펙 031이 가장 먼저 정해야 할 계약이다(§10 F-4).
...
5. **시계**: 시계는 **텍스트 커맨드로 표현 가능**하다(`HH:MM` 문자열) — 별도 커맨드를 만들 필요가 없다.
   단 **시각을 plan 생성 시점에 확정**해야 plan이 결정적이다(§9).
...
| **F-4** | **시계를 인쇄물에 포함하는가** | **포함한다**(미리보기와 일치) — 단 "인쇄 시각"의 의미를 정해야 함 | 현재는 미리보기에만 있어 **본 것과 받는 것이 다르다**(§3.5). 포함하지 않기로 정한다면 미리보기에 그 사실을 **명시**해야 한다 |
| **F-5** | 인쇄에 넣는다면 시계가 가리키는 시각은? | **주문 시각 고정**(plan 생성 시점) | "현재 시각"은 인쇄 파일에서 의미가 없다. 대안: 운영자가 정한 고정 시각(예: 10:10 — 시계 광고 관행) |
...
| **C-8** | 시계 표현 | **별도 커맨드 없이 `draw-text`로** 표현하고, 시각은 **plan 생성 시 확정**해 주입한다(§9.3) |
