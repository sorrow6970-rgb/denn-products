...
## 2026-08-21(14) - 스펙 068 Codex 독립 검수 통과 · 스펙 069 계약 준비

- HEAD=origin `215af5b`, ahead/behind 0/0에서 구현 `31ee0d7`과 기록을 독립 검토했다. 기준
  `c8f54cf` 이후 제품 diff는 허용 신규 2파일(`issue-preparation.ts`와 unit)뿐이고 기존 spec065~067,
  package/lockfile/CSS/Rules/config/UI 제품 diff는 0이다.
- first-await snapshot, catalog detach, proof의 PNG 즉시 복사, SHA/crypto method one-read·receiver 보존
  always-defined adapter, proof→scene→document 순서와 단계별 short-circuit, fresh-copy handle을 대조했다.
  추가 결함은 없다.
- 독립 게이트: targeted **59/59**, space-v2+spaces **364/364**, `node scripts/check.mjs` PASS(unit
  **1935/1935**), 전체 Chromium **151/151**, `git diff --check` PASS, 포트/temp 잔류 0.
...
- 스펙 068 최종 **CODEX_PASSED / DONE**. token/UUID, upload, Firestore create, 실제 Firebase/Rules/
  network/emulator/deploy와 viewer/UI는 계속 NOT IMPLEMENTED / NOT TESTED / 금지다.
- 다음 스펙 069 정본과 handoff를 작성했다. `GG-1=A`의 새 token을 필수 주입 UUID port + lowercase UUID
  v4 검증으로만 분리하며 신규 admin local module/unit 2개만 허용한다. token↔assetId 관계, 스펙 068
  조합과 실제 발급은 열지 않는다.
- 전체 리빌드 진행도는 **76~79% 완료 / 21~24% 잔여, 변동 없음**이다. 스펙 069는 계약만 준비됐고
  작업축 6·7은 움직이지 않았다.
- 상태 `READY_FOR_CLAUDE`, 다음 transition `CLAUDE_IMPLEMENTATION`. NEXT 상단에 Claude Code 전달용
...
## 2026-08-24(7) - 스펙 073 Codex 재검수 · CORRECTION_REQUIRED 라운드 2

- 현재 HEAD=origin `2dd97c4`, ahead/behind 0/0이다. `63a1dec`은 라운드 1 내용 commit,
  `2dd97c4`는 후속 bookkeeping commit이다. 재검수 시작 시 working tree는 기존 Founder/user 보호
  변경뿐이고 staged 0이었다.
- 라운드 1의 private mapping 후보 재분석, `getDoc` timeout 용어 정정, 현재/목표·정적/실행 판정축
  분리는 수용했다. 다음 세 결함 때문에 문서 검수는 아직 통과하지 않았다.
- **보완 1 — primitive evidence:** `storage.emulator.rules:40-45`는 client-denied REC에
  `firestore.exists()`를 쓰고, `firestore.emulator.rules:71-86`은 REC client read를 false로 둔다.
  `cutover-rules.emulator.test.ts:83-96`은 REC 후 Storage upload 성공을 local emulator에서 검증했고
  G-4 정본도 13/13 PASS를 기록한다. primitive는 local emulator VERIFIED이며, V2 mapping 미작성/
  NOT TESTED와 실제 IAM/live NOT TESTED를 분리해야 한다.
- **보완 2 — plaintext surface:** Firestore private mapping은 일반 client read를 막아도 별도 평문
  persistence와 console/Admin SDK/service account/IAM 접근 표면이다. bucket과 동일한 신뢰 수준이라
  새 노출 경로가 아니라는 단정은 근거가 없고 principal/role overlap은 `UNCONFIRMED`다.
- **보완 3 — recId 후보:** no-extra-metadata에서는 object segment `<uuid>.png` 자체를 transform-0 REC
  ID로 쓸 수 있다. 독립 opaque recId는 Storage `customMetadata` pointer 후보도 있으므로 path만으로
  불가능이라 단정하지 않는다. 다만 public metadata 노출·일치 검증·Rules budget을 분석해야 하며
  이는 미승인 schema/Rules 확장, 미작성/NOT TESTED다.
- 상태 `CORRECTION_REQUIRED`, fix round 2, 다음 transition `CLAUDE_DOCUMENT_CORRECTION`이다.
  Founder JJ-1~JJ-7, 제품 구현, Rules 변경, emulator와 다음 스펙은 계속 시작하지 않는다.
- 전체 리빌드 진행도는 **78~81% 완료 / 19~22% 잔여 — 변동 없음**이다. 문서 근거 보완만 요구하며
  제품 작업축 완료량은 증가하지 않는다.

## 2026-08-24(8) - 스펙 073 문서 보완 라운드 2 수행

- 보완 직전 관측 기준 HEAD=origin `2dd97c4`, ahead/behind 0/0. 허용 문서 6개만 수정했다: 조사 보고서,
  `docs/rebuild/specs/073-space-v2-persistence-boundary-investigation.md`,
  `Automation/DENN_AUTOMATION_STATE.md`, `Automation/NEXT_CLAUDE_PROMPT.md`,
  `docs/codex-claude-handoff/CURRENT.md`, 이 live log.
- 제품 코드/test/`storage.rules`/`firestore.rules`/Firebase config/package/lockfile, `apps/**`,
  ...
  **emulator 실행 0**, upload/write/read-back/delete/deploy 0, UID 추측 0, URL 발급 0, UI 연결 0,
  ...
  이번 라운드도 **저장소 파일과 설치 SDK 타입/소스 읽기만** 했다.

**보완 1 — cross-service read primitive 근거 등급 (보고서 §Q7.1.0 신설)**

- 라운드 1은 "client `read:false` 문서를 Storage Rules가 읽을 수 있는지 공식 인용 미취득"을 이유로
  **primitive 자체를 `UNCONFIRMED`로 남겼다. 그 분류가 틀렸다** — 이 저장소에 공식 인용보다 강한
  **local 실행 증거**가 이미 있다.
- 네 층위로 분리했다:
  - **① 공식 지원** — G-4 결정 정본 §4가 `firebase.google.com/docs/storage/security/rules-conditions`를
    직접 인용(`firestore.get()`/`firestore.exists()` 사용 가능). 같은 §4가 제약 4개도 인용한다 —
    default DB 한정 · **단일 Rules 평가 문서 2개** · Firestore quota/과금 포함 · IAM 활성화/해제.
  - **② local emulator VERIFIED** — `storage.emulator.rules:40-45`가
    `firestore.exists(/databases/(default)/documents/rebuildAdminStateObjects/$(objectId))`로 Storage
    create를 게이팅하고, `firestore.emulator.rules:71-86`이 같은 REC에 `allow read, update, delete:
    if false`를 건다. `packages/firebase/src/admin-write/cutover-rules.emulator.test.ts:83-96`이
    **REC을 `setDoc`한 뒤 Storage upload 성공**, 같은 경로 재업로드·`deleteObject`·비승인 UID 거부를
    `demo-denn-emulator`에서 검증한다. G-4 정본 §12가 2026-08-14 **13/13 PASS**를 기록한다.
  - **③ V2 전용 mapping Rules** — 미작성 · `NOT TESTED`.
  - **④ 실제 Firebase / IAM / live** — `NOT TESTED`.
- ②가 말해 주는 것: 그 시나리오에서 create 규칙이 REC 존재를 요구했고 **upload가 성공**했으며 REC의
  Firestore client `read`는 `false`다 ⇒ **Storage Rules의 service-side cross-product 평가는 대상
  문서의 Firestore *클라이언트* read 권한에 좌우되지 않는다.**
- ★ **"우회(bypass)" 표현을 폐기**했다. Firestore client read 권한은 *클라이언트 요청*이 그 문서를
  읽을 수 있는지를 정하고, Storage Rules의 `firestore.get()/exists()`는 *규칙 평가 과정에서 서비스가
  수행하는 별개 조회*다 — 주체도 평가 경로도 다르다.
- ②는 `.json` admin-state 경로 검증이며 **이번 세션에서 재실행하지 않았다**(emulator 금지). 기록된
  PASS와 테스트 소스를 근거로 삼는다.

**보완 2 — privileged plaintext surface (보고서 §Q7.1 보안 항목 재정정)**

- 라운드 1이 덧붙인 *"버킷 객체 자체와 같은 신뢰 수준이므로 새로운 노출 경로는 아니다"* 단정을
  **폐기**했다. 근거가 없다.
- private mapping은 **token 또는 asset↔document 관계의 평문 사본**을 만든다. 현재 V2 설계에서 그
  관계는 **어디에도 평문으로 존재하지 않는다**(암호문 안에만). 매핑을 두는 순간 **없던 평문 사본이
  생긴다.**
- 그 사본에는 **Firebase console · Admin SDK · service account · IAM**이라는 별도 접근 표면이 붙고,
  bucket 접근 주체와 **정확히 같은 principal/role 집합인지 확인하지 않았다**(프로젝트 IAM 구성을
  읽지 않았고 live 접근이 금지돼 읽을 수도 없다).
- ⇒ 정확한 진술: **새로운 privileged plaintext surface이며 overlap은 `UNCONFIRMED`.**
  이것만으로 후보를 금지하지도 승인하지도 않고 **Founder 보안 tradeoff**로 남겼다(JJ-5 반영).
- 유지되는 것: 일반 클라이언트에게 token/관계를 노출하지 않는다는 점, 초판의 "보안 모델 충돌" 단정이
  과장이었다는 점.

**보완 3 — REC ID 후보 완결성 (보고서 §Q7.1.1 신설)**

- 라운드 1의 *"opaque recId는 성립하지 않는다 — path가 `{assetId}.png`로 고정됐으므로 Storage
  segment가 곧 assetId"* 확정을 **폐기**했다. 두 가지가 부정확했다 — wildcard가 잡는 값은 **bare UUID가
  아니라 세그먼트 전체 `"<uuid>.png"`**(admin-state `"<uuid>.json"`과 동일 구조이며 G-4 §8.1이
  라운드 2 교정 3으로 이미 한 번 바로잡은 지점)이고, **"추가 metadata가 없다면"** 이라는 전제를
  빠뜨렸다.
- **(c1) transform-0** — REC doc id = Storage 세그먼트 그대로 `"<uuid>.png"`, `$(assetId)`를 변환 없이
  보간. **admin-state G-4 §8.2가 확정한 바로 그 패턴**이고 문자열 파싱·분해·연결 **0**(G-4 §8.4 원칙
  준수). 조회 패턴 자체는 §Q7.1.0 ②로 **local emulator VERIFIED**(`.json`에서). 형식은
  `'^…[.]png$'` 정규식으로 강제 가능(`firestore.rules:76-79`가 `.json` 선례).
  한계: **opaque하지 않다** — 다만 assetId는 public-read path에 이미 있어 비밀이 아니므로 식별
  목적에는 무해하다. ⇒ **성립한다.**
- **(c2) 독립 opaque recId + Storage `customMetadata` pointer** — 근거는 설치
  `@firebase/storage/dist/storage-public.d.ts` `:500`(`uploadBytes(ref, data, metadata?:
  UploadMetadata)`), `:515`(`UploadMetadata extends SettableMetadata`), `:277`·`:301-303`
  (`customMetadata?: { [key: string]: string }`), `:56`(`FullMetadata extends UploadMetadata`).
  **create와 metadata를 같은 `uploadBytes` 호출에 넣을 수 있다**(별도 `updateMetadata` 불필요 —
  나중에 붙이면 창이 생기고 update를 열면 불변 계약이 깨진다).
  그러나 ⓐ Rules의 `request.resource.metadata`/`resource.metadata` 지원과 정확한 접근자는
  **저장소 선례 0건 · `UNCONFIRMED`**, ⓑ 값이 문자열뿐이라 exact key/format 설계 필요,
  ⓒ recId↔assetId 일치 교차 확인까지 넣으면 **문서 접근 한도 2를 넘는다**((c1)은 2 안에서 끝난다),
  ⓓ ★ **public-read라 경로를 아는 누구나 `getMetadata()`로 `customMetadata`를 읽는다** ⇒ **recId를
  비밀로 취급할 수 없고 token 삽입은 명백히 금지**, ⓔ `updateMetadata` 차단이 목표 문구에 **누락**된
  계약 공백, ⓕ **GG-4=A가 승인한 적 없는 schema/Rules 확장**.
  ⇒ **논리상 가능하나 (c1)보다 명백히 비싸고 위험하며 미작성 · `NOT TESTED`.** path만 보고
  "불가능"이라 단정하지 않되 "가능"이 "권장"이나 "안전 PASS"를 뜻하지도 않는다.
- 두 후보 모두 **확정 orphan을 증명하지 못한다** — 단조값 부재 문제는 REC ID 형태와 무관하다.
- 연쇄 경로 보간(`get()` 결과를 다른 경로에 보간)은 여전히 `UNCONFIRMED`이며, **admin-state SDC′는
  고정 경로와 path 변수만 써서 연쇄를 한 번도 쓰지 않았으므로 §Q7.1.0 ②의 VERIFIED 증거가 이
  부분은 덮지 않는다**는 사실을 명시했다.

**보완 4 — commit 자기참조 추적 중단**

- 라운드 1까지는 내용 commit 뒤 "해시를 적어 넣는" bookkeeping commit(`534c26f`, `2dd97c4`)을 하나 더
  만들었다. **라운드 2부터 그 추가 commit을 만들지 않는다.**
- 이유를 숨기지 않고 적었다 — **commit은 자기 자신의 해시를 내용에 담을 수 없다.** 그 한계를 메우려
  라운드마다 commit을 두 배로 늘리면 기록이 자기참조로 꼬인다. 대신 상태 문서에는 **① push 후
  `HEAD=origin`과 `ahead/behind 0/0`을 실제로 검증했다는 사실**과 **② 라운드 2 내용 commit이
  무엇인지**를 나눠 적고, 해시 정본은 **git 이력과 세션 보고**에 둔다.
- 이력: `f1f5d20`(초판 내용) → `534c26f`(bookkeeping) → `63a1dec`(라운드 1 내용) →
  `2dd97c4`(bookkeeping) → **라운드 2 내용 commit 1개**.

- JJ-5 선택지도 다시 고쳤다: A(삭제 보류) · B((c1) transform-0 mapping) · B′((c2) customMetadata) ·
  C(backend) **어느 선택도 확정 orphan을 증명하지 못하며 어느 것도 삭제 승인이 아니다.** B에는
  "primitive는 VERIFIED지만 연쇄 보간 `UNCONFIRMED`, 새 privileged plaintext surface(overlap
  `UNCONFIRMED`)"를, B′에는 "GG-4 미승인 확장 + public 관측"을 조건으로 붙였다.
- 게이트: `git diff --check` PASS, 허용 6개 문서 외 diff **0**(제품·Rules·config·lockfile·보호 대상
  무변경 확인). 문서 전용 단위라 실행 게이트는 없으며 unit/E2E/typecheck/build/emulator를 **하나도
  돌리지 않았고 돌렸다고 기록하지 않는다.**
- 전체 리빌드 진행도는 **78~81% 완료 / 19~22% 잔여 — 변동 없음**이다. 라운드 2에서 primitive 하나가
  `UNCONFIRMED` → **local emulator VERIFIED**로 올라갔지만, 이는 **admin-state에서 이미 검증돼 있던
  사실을 이 문서가 잘못 낮춰 적었던 것을 바로잡은 것**이지 이번에 새로 검증한 것이 아니다. 진행도를
  올릴 근거가 되지 못한다. 오히려 §Q7.1·§Q7.1.1이 선택지마다 붙는 조건을 더 분명히 했으므로
  **작업축 6의 잔여 난이도는 줄지 않았다.**
- 상태 `READY_FOR_CODEX`, fix round 2/3, 다음 transition `CODEX_RE_REVIEW`. Founder JJ-1~JJ-7 선택,
...
## 2026-08-24(13) - 스펙 073 문서 보완 라운드 4 (Founder 예외 승인)

- 보완 직전 관측 기준 `HEAD=origin=a6ad189`(Founder 라운드 4 예외 승인 commit), ahead/behind 0/0.
  승인문이 적은 `dc6fe11`은 그 **직전 commit**이고 그 위에 승인 commit `a6ad189`가 얹혀 있다 —
  기준선은 정합하며 라운드 4는 `a6ad189`에서 출발했다.
- Codex 최종 재검수의 **근거 정정 1건만** 반영했다. 라운드 1·2·3에서 통과한 내용은 **하나도 되돌리지
  않았다.** 허용 문서 6개(조사 보고서 · spec073 · STATE · NEXT · CURRENT · live log)만 수정했고
  **내용 commit 1개**만 남기고 self-hash bookkeeping commit은 추가하지 않았다.

**보완 1 (유일한 정정) — Storage Rules object metadata 표면의 근거 등급**

- 라운드 3까지 보고서 §Q7.1.1·§4·§7은 *"이 저장소에 metadata를 읽는 Rules 선례가 0건이므로
...
- **폐기한 것은 딱 둘** — ① *"저장소 선례 0건이므로 공식 지원도 `UNCONFIRMED`"*, ② *"(c1)만 Rules
  표면 근거 등급을 확보했고 (c2)는 갖지 못한다"* 는 비교.
- ⇒ (c2)에 남는 실제 차이는 **둘**이다: **목표 public-read 구현 시 recId가 공개 식별자**,
  **GG-4 미승인 schema/Rules 확장**. 여기에 **(c2)에만 붙는 설계 요건**(허용 키 exact key/format
  검증 · mapping ↔ assetId 교차검사)이 남는다.

**유지한 것 (되돌리지 않음)**

- V2 전용 Rules **미작성**, exact key/format·assetId 교차검사 **설계 필요**.
- **emulator/runtime `NOT TESTED`** — 이 저장소에 metadata를 읽는 Rules 선례는 여전히 0건이고,
  공식 지원은 **정적 근거**이지 실행 검증이 아니다.
- 목표 public-read 구현 시 **recId가 관측되는 설계 귀결**(recId를 secret으로 설계하지 않고 token을
  `customMetadata`에 넣지 않는다).
- **GG-4 미승인 schema/Rules 확장**, 실제 Firebase/IAM/live **`NOT TESTED`**.
- **(c1)·(c2) 모두 확정 orphan 미증명**과 **O-3 삭제 보류**.
- 라운드 2·3 통과 내용 전부 — primitive 4층위 분류, privileged plaintext surface(overlap
...
- 보고서: §1.4(신설) · §Q7.1.1 (c2) 「Rules 표면」 행과 마무리 문단 · §Q7.1 선택지 표(V2-2″ 행) ·
  §Q7.1 UNCONFIRMED 목록 · §Q10 JJ-5 행 · §4(UNCONFIRMED → 「이미 검증된 것」 이동, `NOT TESTED`
  보강) · §7 진행도 · 머리말(상태 · commit 표 · 라운드 4 변경 요약).
- spec073: 상태 헤더, 라운드 2 기록의 superseded 주석, 라운드 3 (c2) 재평가 ① 폐기 표시,
  `DONE (Claude) — 보완 라운드 4` 신설.

**게이트·경계**

- `git diff --check` PASS, 허용 6개 문서 외 diff **0**(제품·Rules·config·lockfile·보호 대상 무변경
  확인). 문서 전용 단위라 실행 게이트는 없으며 unit/E2E/typecheck/build/emulator를 **하나도 돌리지
  않았고 돌렸다고 기록하지 않는다.**
- 실제 Firebase/project/bucket/Firestore/network/live 접근 **0**, emulator 실행 **0**,
  upload/write/read-back/delete/deploy **0**, UID 추측 **0**, URL 발급 **0**, UI 연결 **0**,
  자동화·반복 작업 **0**. 이 세션은 network 접근이 금지되어 **공식 문서 URL을 직접 fetch하지 않았다**
  — 인용은 Codex 재검수가 제시한 것이며 보고서 §1.4에 그 사실을 명시했다.
- 전체 리빌드 진행도는 **78~81% 완료 / 19~22% 잔여 — 변동 없음**이다. 라운드 4는 **근거 등급 하나를
  바로잡은 문서 정정**이며, 라운드 2의 primitive 정정과 같은 성격으로 **새로 검증한 것이 아니라 잘못
  낮춰 적었던 것을 고친 것**이다. 규칙은 여전히 미작성·실행 검증 0이므로 **작업축 6의 잔여 난이도는
  줄지 않았다.**
- 상태 `READY_FOR_CODEX`, fix_round **4(Founder 예외 승인 라운드) 수행 완료**, 다음 transition
...
## 2026-08-26 - 스펙 078 Codex 재검수 통과 · DONE

- 재검수 기준 `HEAD=origin=6742f3f`, ahead/behind 0/0. correction code/test commit `bed9106`과 결과 문서를
  스펙 029·030·064·078 계약에 대조했다.
- 추가 결함 0. probe/final plan의 geometry·scale·quarter-turn 동일, builder-derived rotated maxPan,
  `toLogicalTransform()`을 통한 normalized→logical 단일 환산을 확인했다.
- 독립 PASS: targeted **29/29**, 전체 check(unit **2153/2153** 포함), 고객 entry
  `index-6js4DafP.js` / 322,018 bytes / SHA-256
  `A9360EFFBC204A2291AF66088840F7C7E58E97E8A29BE36B0669FC42E55E8159`, correction `git diff --check`,
  허용 8개 경로, 검사 포트 잔류 0.
- 판정 `CODEX_PASSED / DONE / LOCAL_VERIFIED / NO_UI / NO_NETWORK`. 전체 Chromium E2E·emulator는
  계약대로 NOT RUN이며 실제 Firebase/network/live/deploy·production UI/asset adapter 연결은 0.
...
## 2026-08-27 - 스펙 080 보완 라운드 2 (clean checkout EOL 재현성)
...
- 기준 `HEAD=origin=3b7c72c`, ahead/behind 0/0. Codex 재검수 문서 commit `1ae514a`, 보완 commit
  ...
  되돌리지 않았다.
- **결함 재현.** `biome format`을 세 파일에 직접 돌려 확인했다 — 보고된 diff는 전부 `␍`(CR)뿐이고
  코드 내용 차이는 0이었다. 실패 원인은 line ending 하나이며 Codex 판정이 정확하다.
- **원인.** system `core.autocrlf=true`이고 기존 `.gitattributes`는 `*.bat`/`*.cmd`/`*.ps1`만 CRLF로
  고정한다. commit blob은 이미 LF지만 Windows checkout이 worktree 사본을 CRLF로 만들고 Biome은 LF로
  포맷하므로, 라운드 1의 전체 check PASS가 clean checkout에서 재현되지 않았다.
- **보완.** `.gitattributes`에 **정확히 세 경로만** `text eol=lf`로 고정했다 —
  ...
  않았다 — 저장소 전체 line-ending 정책은 별도 결정이고 스펙 080 보완이 대신 정할 일이 아니다.
  이유는 파일 주석으로 남겼다. 이어서 세 파일의 worktree 사본을 CRLF→LF로 변환했다.
- **검증.** `git ls-files --eol`이 세 파일 모두 **`i/lf w/lf attr/text eol=lf`**를 보고하고
  `biome format`도 세 파일 clean이다.
- **semantic diff 0.** 보완 commit의 staged diff는 `.gitattributes | 9 +++++++++` **한 파일뿐**이고
  세 파일은 `git diff --exit-code` clean이다. 고객 entry 해시가 Codex 기록값과 **동일**하다 —
...
- **실측.** 전체 `node scripts/check.mjs` **PASS**(format 포함, unit **2281/2281**), targeted Chromium
  `space-production-route.spec.ts` **14/14 PASS**, `git diff --check` PASS, 검사 포트
  4183/4184/4185/8080/9099/9199 잔류 **0**, 강제 종료 0. Playwright `test-results/`는 제거했다.
- **무변경 확인.** 고객 CSS `index-BjqjBda8.css`(19,381) · admin entry `index-D0XOQpRL.js`(226,201) ·
  admin CSS `index-DJ_z3tK1.css`(9,146) 모두 SHA-256까지 동일. spec-080 PNG 2개는 E2E 재실행으로 다시
  생성됐으나 **byte-identical**이라 diff 0, spec-063 PNG도 diff 0이다.
- 변경 파일은 `.gitattributes`와 spec-080 상태 문서뿐이다. 제품 semantic diff·PNG 재생성 diff·
  Rules/firebase/package/lockfile 변경 **0**이고 보호 대상과 기존 user working-tree 변경은 읽기만 했다.
- **full Chromium suite는 보호 spec-018 PNG 때문에 계속 NOT RUN**이며 PASS라고 기록하지 않는다.
  actual Firebase/network/live/CORS/deploy/UID는 **0 / NOT TESTED**다.
...
## 2026-08-27 - 스펙 082 보완 라운드 2 (Founder NN-2=A) — 전체 E2E 160/160
...
- 기준 `HEAD=origin=60507b3`, ahead/behind 0/0. Codex 재검수·NN-2 문서 commit `60507b3`, 보완 commit
  ...
  고쳤고 **제품 코드는 한 줄도 바꾸지 않았다**.
- **전체 Chromium E2E가 160 passed / 0 failed다** — 기존 159건 + 이번에 추가한 call-surface 테스트 1건.
  라운드 1까지 남아 있던 **158/1이 해소**됐다.
- **결함 — 검사가 결정할 수 없는 것을 묻고 있었다.** `bundle.includes("uploadBytes")`는 번들된
  Firebase 제품이 **모듈 전체를 싣는다**는 사실 때문에 "이 앱이 쓰기를 하는가"가 아니라 "Storage
  SDK가 존재하는가"를 측정했다. 실제로 그 이름들은 vendor chunk의 **export 맵**
  (`Jt as uploadBytes`, `en as listAll` …)과 SDK 내부 **오류 라벨**(`_throwIfRoot("uploadBytes")`)에
  있을 뿐 고객 호출부가 아니다. 게다가 같은 목록이 `getStorage`도 금지했는데 그건 스펙 079(MM-1=A)가
  **승인한 바로 그 호출**이라, 079 이후에는 이 검사가 통과할 수 없는 상태였다.
- **보완 — 결정 가능한 곳에서 경계를 본다.** 번들 substring 목록에서 Storage 이름 6개를 빼고 vendor가
  만들지 않는 app-level 문자열만 남겼다(`admin-read` · `ADMIN_STATE_OBJECT_PATH` ·
  `admin/state.json` · `onAuthStateChanged` · `signInWithEmailAndPassword`). 라운드 1의 `getAuth`
  whole-identifier 검사, positive marker(`denn-space-viewer`/`getFirestore`/`getDoc`), default route
  external request 0 검사는 **그대로 유지**했다. 대신 신규 테스트
  `the customer app's own Storage call surface stays read-only`가 고객의 **자기 소유 production
  source**를 검사한다 — `apps/mockup/src`(unit test와 `e2e/` 제외) + 고객이 실제로 import하는 유일한
  `@denn/firebase` subpath인 `packages/firebase/src/space-read`(test 제외), 총 **58파일**. 주석 제거 후:
  ① `@denn/firebase/space-write`·`admin-read`·`admin-write` **import 0**(이 단언이 package 절반을
  `space-read`로 한정한 근거이며, 고객이 다른 subpath에 손대면 **먼저 깨진다**)
  ② `uploadBytes`·`uploadBytesResumable`·`uploadString`·`updateMetadata`·`deleteObject`·`list`·
  `listAll`·`getDownloadURL`·`getBlob`·`getStream`의 **호출**(`\b이름\s*\(`) **0**
  ③ `getStorage`·`ref`·`getMetadata`·`getBytes` 호출이 **실제로 존재**(이게 없으면 Storage 호출이 스캔
  밖으로 옮겨가도 테스트가 계속 통과해버린다).
- **이빨이 있다는 측정 증거.** 같은 검사를 **admin write surface**(`apps/admin/src` + `space-write`)에
  겨누면 `uploadBytes`가 **FAIL 1**로 잡힌다 — 실제 쓰기를 검출한다. Storage vendor chunk
  `index.esm-DtyxGWvl.js`의 `.이름(` 호출 형태는 **0**이라 export 맵·오류 라벨이 호출부로 위장할 수
  없다. 고객 app chunk(`index-*.js`, `SpacePostAuthFrameView-*.js`)는 허용 4종만 쓰고(`getStorage` 1 ·
  `ref` 2 · `getMetadata` 1 · `getBytes` 1 · `connectStorageEmulator` 1) 금지 **0**이다. 번들 문자열이
  아니라 **호출 형태**를 보므로 vendor에 오염되지 않고, source를 보므로 bundler chunk 분할 방식에
  의존하지 않는다.
- **실측.** targeted `admin-auth-read.spec.ts` Chromium **4/4 PASS**, **전체 Chromium E2E 160 passed /
  ...
  4183/4184/4185/8080/9099/9199 잔류 **0**, `test-results`/temp 잔류 0.
- **약화 없음.** 테스트를 삭제하지 않았고 E2E 예외로 처리하지도 않았다. Storage 쓰기·열거·다운로드
  경계는 오히려 **더 강해졌다** — 예전엔 vendor 문자열 유무만 봤지만 이제 실제 호출과 import 경계를
  본다. Auth·admin private path·external request 0 검사는 그대로이고, 스펙 079/080의 read-only Storage
  승인은 보존했다.
- E2E가 다시 쓴 보호 spec-018 PNG 2개는 **stage/commit/restore하지 않고 dirty 그대로** 뒀고 기존 user
  dirty 파일도 그대로다. 실제 admin issue UI와 다음 스펙, 자동화는 시작하지 않았다.
...
## 2026-08-27 - 스펙 082 보완 라운드 5 (Founder NN-4=A 예외) — regex를 버리고 compiler scanner로
...
- 기준 `HEAD=origin=87923e6`, ahead/behind 0/0. Codex 재검수·NN-4 문서 commit `a1d3aaa`, 보완 commit
  ...
  코드·승인된 read-only Storage 연결·**신규 의존성은 0**이다(`typescript` 7.0.2는 이미 root
  devDependency, `package.json`/lockfile 무변경).
- **Codex 지적은 옳고 반복의 원인도 옳게 짚었다.** 라운드 1~4는 regex로 형태를 하나씩 막았다 — 호출 →
  alias → property → named import. 그리고 다섯 번째가 또 나왔다: `const { list: l } = storage`,
  `const { list } = storage`. **형태 목록이 문제가 아니었다** — regex는 구문을 못 보니 매 라운드가
  "다음에 누가 뭘 쓸까"라는 추측이었고 네 번 틀렸다.
- **추측을 그만두고 컴파일러처럼 읽는다.** 저장소에 이미 있는 TypeScript scanner로 토큰화하고, 형태
  목록 대신 질문 두 개를 던진다.
- **① SDK 각 모듈이 실제로 무엇을 건네는가 — 모듈별 allowlist와 정확히 같아야 한다.**
  ...
  **양방향**이라 목록 밖(=아무도 금지할 생각을 못 한 능력 포함)은 실패하고, 목록이 비어도 실패해
  승인된 read 경로가 조용히 사라질 수 없다. reader가 **설명 못 하는 형태**(computed member,
  namespace를 값으로 전달, 모듈과 짝지을 수 없는 binding)는 **침묵이 아니라 실패**로 보고된다 — 이게
  allowlist를 "긴 목록"이 아니라 **닫힌 집합**으로 만든다.
- **② 금지 이름이 어떤 구문 위치에서든 도달 가능한가 — property · string member · braced clause.**
  마지막 하나가 destructuring·named import·re-export alias를 함께 덮는다(셋 다 `:`/`as` **왼쪽**에서
  이름을 가져오므로). `list` bare identifier 면제는 유지되고 비용은 여전히 0이다.
- **parser 문맥이 필요한 토큰 두 개를 parser처럼 재스캔한다** — `/`(나눗셈 vs 정규식)와 template
  substitution을 닫는 `}`. 두 번째를 빼먹으면 scanner가 **파일 나머지를 template 텍스트로 삼킨다**:
  실제로 초기 구현에서 `proof-sdk-facade.ts`가 255 토큰에서 끊겨 `storage.getStorage` 이하가 전부 안
  보였다(전체 35,701 → 49,364 토큰). reader가 눈머는 것이 바로 이 검사가 막으려는 실패라서,
  **scanner가 전진을 멈추면 throw**한다.
- **이빨이 있다는 실측 — 합성이 아니라 실제 코드.** 같은 reader를 admin write surface
  (`apps/admin/src`+`space-write`+`admin-read`, 35파일)에 겨누면 `firebase/auth`를 **승인 안 된 모듈**로,
  세 제품에서 **승인 밖 멤버 11개**(`uploadBytes`·`setDoc`·`getDocFromServer`·
  `connectFirestoreEmulator`·`getAuth`·`signInWithEmailAndPassword`·`signOut`·`onAuthStateChanged`·
  `setPersistence`·`browserLocalPersistence`·`connectAuthEmulator`)를 잡아내고, 금지 이름 검사도
  `space-write/sdk-facade.ts`의 `uploadBytes`를 검출한다. 고객 surface 66파일에서는 설명 못 한 형태
  **0**, allowlist 밖 **0**, 금지 이름 **0** — 두 surface 합쳐 **실제 101파일**에서 reader가 모든 형태를
  설명했다.
- **self-check도 같은 파일에서 유지·확장.** Codex의 두 destructuring 케이스, alias/property/bracket/
  직접 호출, named import·re-export alias는 **잡히고**; `import { templateList as list }`·지역 `list`·
  주석 속 이름·문자열 리터럴 `"uploadBytes"`는 **안 잡히며**; `storage[name]()`과 `handOff(storage)`는
  **unaccounted로 실패 보고**됨을 증명한다.
- **실측.** `admin-auth-read.spec.ts` Chromium **5/5 PASS**, **전체 Chromium E2E 161 passed /
  0 failed**, 전체 `node scripts/check.mjs` **PASS**(format·lint·typecheck 7개·unit **2409/2409**(89
  파일)·build 2개), **통제 빌드 대조에서 산출물 16개 전부 byte+SHA-256 동일**, `git diff --check`
  PASS, 변경 경로는 허용 **한 파일뿐**, EOL `i/lf w/lf`, 검사 포트 4183/4184/4185/8080/9099/9199 잔류
  **0**, `test-results`/temp 잔류 0.
- **약화 없음.** 테스트 삭제·skip·E2E 예외는 없다. Auth whole-identifier, admin private path marker,
  positive marker, runtime external request 0 단언은 그대로다.
- E2E가 다시 쓴 보호 spec-018 PNG 2개는 **stage/commit/restore하지 않고 dirty 그대로** 뒀고 기존 user
  dirty 파일도 그대로다. 실제 admin issue UI와 다음 스펙, 자동화는 시작하지 않았다.
...
## 2026-09-02 - Claude 스펙 085 고객 composer 결과 우선 작업대 (F-1 해소)
...
## 2026-09-03 - Claude 스펙 087 고객 Space 인증 후 머리말 정리 (F-3 해소)
...
- **F-3 해소.** 인증 **전**은 무변경이다 — badge·`<h1>내 공간 시안 확인</h1>`·`담당자에게 전달받은
  비밀번호를 입력하세요.`·form·상태 문구·`role`/`aria-live` 그대로이고
  `space-v2-password-gate-390x844.png`는 sha256 `67a1433c…`로 바이트 동일이다. 인증 **후**에는
  `readyBodyOwnsTitle`(= `status === "ready"` 이고 해당 결과 renderer가 주입됨)일 때 게이트가
  badge·`<h1>`을 렌더하지 않고, 비밀번호 안내는 `ready`이면 **무조건** 사라진다. 판정은
  `renderReadyBody`가 이미 분기하는 **바로 그 조건**이라 둘이 어긋날 수 없고 자식 DOM을 보지 않는다.
  결과 renderer가 없는 spec 061 fallback(`pendingNotice`)은 자기 제목이 없으므로 머리말을 유지한다.
- **결과 화면 3곳이 제목을 갖는다.** `SpaceV2ProofView`(`space-v2-proof-title`), V1 액자 뷰
  ...
  **문구·class·id 그대로**라 세 `aria-labelledby`가 모두 실재 요소로 해석되고 두 badge도 유지했다.
  **새 고객 문구 0**, CSS 수정 **0**(`.denn-shell__inner h1`(0,1,1)이 `.denn-space-blocked__title`(0,1,0)의
  font-size·margin을 이기고, class가 주는 `word-break: keep-all` / `overflow-wrap: anywhere`는 살아남아
  320px에서도 넘치지 않는다).
- **★ 계약 보완 판단 요청 2건.** ① **허용 파일 1개 누락** — 계약은 `SpacePostAuthFrameView`만 적었는데
  제품 V2 route가 실제로 렌더하는 결과 화면은 `apps/mockup/src/space-v2/SpaceV2ProofView.tsx`다
  (`App.tsx:65`). 빼면 §3의 "인증 후 `<h1>` 정확히 1개"가 F-3이 측정된 바로 그 화면에서 성립하지 않는다
  (신규 E2E가 `h2: 내 공간 시안`으로 실패했다). §2가 이미 "V2 액자 뷰가 단독 제목을 갖는다"를 요구하므로
  의도 안이라 보고 포함했다. ② **허용 PNG 4장 누락 — stage하지 않았다.** canonical이 같은 두 화면의 다른
  tracked baseline `spec-063/space-v1-blocked-{desktop-1280x800,mobile-390x844}.png`와
  `spec-080/space-v2-viewer-{desktop-1280x800,mobile-390x844}.png`도 다시 썼다. 인과는 확인했다 —
  spec-080 두 장은 spec-084 viewer 두 장과 **sha256이 서로 같은 동일 캡처**이고(변경 후에도
  `c84d8f16…`/`0ce9e921…` 쌍 유지), spec-063은 같은 V1 차단 화면이다. 계약의 "허용 3장 밖 tracked PNG가
  바뀌면 stage하지 말고 보고한다"에 따라 unstaged로 두었다. 허용 목록 추가 여부는 Codex 판단이다.
- **검증.** `node scripts/check.mjs` PASS(format·lint·typecheck, unit **2510/2510** · 92 파일, build
  2개; 신규 unit 8). canonical `node scripts/e2e-run.mjs` **Chromium 223 passed / 0 failed / 0 skipped /
  0 retry**(기존 220 + 신규 3). 신규 E2E 3건은 `390x844`·`1280x800`에서 인증 전 heading 목록
  `["h1: 내 공간 시안 확인"]`·안내 표시를 확인한 뒤 인증 후 목록이 정확히 `["h1: 내 공간 시안"]`이고 게이트
  제목·비밀번호 안내가 DOM에 0개, badge 유지, `aria-labelledby="space-v2-proof-title"`가 `H1`로 해석,
  horizontal overflow 0, axe serious/critical 0, console error/warning 0, pageerror 0, 외부 요청 0을
  단언하고, V1 링크에서는 차단 안내가 유일한 `h1`이며 `role="alert"`·재시도 버튼 0이 유지됨을 단언한다.
  기존 `getByRole("heading", { name: "내 공간 시안" })`·`저장된 시안 · 열람 전용` 단언은 **무수정 PASS**다.
  timeout·retry·skip·screenshot tolerance 추가 0. `git diff --check` PASS, 포트 4183/4184/4185/8080/
  9099/9199 LISTENING 0, `denn-e2e-*`·`playwright-report`·`debug.log` 잔류 0.
- **구현 중 자체 수정 1건.** 신규 E2E가 처음 `aria-labelledby="space-frame-title"`(V1 액자 뷰의 id)을
  확인해 실패했다. V2 route의 결과는 `SpaceV2ProofView`(`space-v2-proof-title`)이므로 test를 실제 화면에
  맞게 고쳤다. 제품 의미는 바뀌지 않았다.
- **증거.** 허용 3장을 canonical이 다시 썼고 직접 열어 확인했다 — V2 뷰어는 badge 1개 + 제목
  ...
  무변경. `measurements.json`의 Space 4항목은 overflow false·axe serious/critical 0·smallTargets 0이다.
  감사 보고서 §11에 F-3 해소 addendum을 남겼고 §9 F-1·§10 F-5·F-6 철회·F-8 재분류는 되돌리지 않았다.
...
- **보호 대상.** design README·spec 038·`packages/render/src/plan/index.ts`·`pnpm-workspace.yaml`·
  `AGENTS.md`·`taste-v2/**` hash 동일, restore·checkout·stage·commit 0. spec 018 PNG는 canonical이 다시
  썼고 stage하지 않았다: desktop `d0a0aa52…` → `bd5ff207…`, **이번엔 mobile도** `6bdcb88c…` →
...
## 2026-09-07 - spec093 전송 확인 · spec094 복구 안내 구현/검수

- 범위승인기록a80cd9c와0931e322c1/cfb1a32 정상push성공,cca5a16..a80cd9c.094시작HEAD=origina80cd9c·0/0.
- 094계약을먼저작성·검토한뒤제품의save-error안내만canSave에맞춰분리.새권한/자동retry/merge0.
  코드·증거11파일38257ef:제품1/신규SSRtest1/기존E2E기대문구2개1파일/증거8.093 F-9해소.
- check전체PASS:단위95파일2526/2526(기존2517+9,3.39초). canonical1회267 tests,exit0,last-run passed.
  도구출력이잘려앱의해당명령기록에서완료/exit0/전체56,626ms를재확인했다.순수E2E시간은확인불가.
  재실행/timeout/retry/worker변경0. 전역프로세스읽기권한거절은우회/재시도하지않았고kill0.
- 변경6PNG전부직접확인.측정18건PASS,활성12/비활성78,overflow/44px미만/axe중대/console/외부시도0.
  PNG18hash불일치0,고객entrySHA불변.운영자SHA B0A1F85F9271E4A929D2F6AB0F20BB0D0BFDADDA211533DDD675C8FB85711246.
- 기존결과/보호100개중90동일,허용증거8+spec018PNG2예외만hash변경.093나머지12PNG/092·084증거불변.
  보호20개중18동일,PNG2복원/stage/commit0.정확hash는094검수참조.포트6개잔류0,stagingeUpy5K부재.
- 종료9문서별도커밋·동일원격push단계.별도roadmap/spec091handoff2와보호/userdirty는제외한다.
  같은Codex검수이지독립검수아님.실사용자/실기기/스크린리더/실제인증·Firebase·운영복구NOT TESTED.
  운영전환/배포/삭제/발행/자동화0.다음은잔여기능·기존승인읽기전용대조,새선택필요시STOP.
- 전체85~88%관리추정유지,잔여100-88=12~100-85=15%.실측완료율·최종스펙수/완료일확인불가.

## 2026-09-07 - spec094 정상 전송 영수증 · 잔여 범위 초기 대조

- 코드/증거38257ef(11파일)+종료5a1abe6(9문서) 정상push성공 a80cd9c..5a1abe6.
  확인시HEAD=origin5a1abe6,ahead/behind0/0.보호/별도사용자문서2개stage/commit0.
- 전송영수증 STATE/NEXT/CURRENT/live/094handoff5문서만추가동기화.제품코드/테스트재실행0.
- 레거시인벤토리·현재App2개·033제외범위·기존로드맵을읽어룸/가이드/주문·카카오/전체catalog/발행의
  완료를아직주장할수없음을재확인.초기근거표는094handoff,전수조사나새출시선택승인은아니다.
...
## 2026-09-07 - spec096 정상 전송 영수증

- 코드·시험·합성증거12파일 b03f8d4 + 종료8문서5460fb3 정상push성공 f7c9ecd..5460fb3.
  확인시HEAD=origin5460fb3,ahead/behind0/0.총변경20경로=12+8,허용범위일치.
...
## 2026-09-09 - spec105 push 완료 · 다음 조사 루틴 승인

- 사용자 `응 큰 문제없다면 별도 승인없이 모든스팩 진행해`로 직전 코드5/문서7 및 동일문서 최종기록 전송 승인.
  중요 제품 선택/권한/충돌 외 후속 루틴도 계속하도록 지시받았다. 예약자동화는 만들지 않는다.
- origin/rebuild/modern-studio 일반push d063ae1..bc1c240 성공,77773db 코드와bc1c240 문서 반영.
...
- 문서4개2662e40,문서7개9839f2c 분리commit 성공. 기존코드bfd1e55/3c13334와함께
  일반push 585f88b..9839f2c 성공. HEAD=origin9839f2c·ahead/behind0/0 확인.
- 정확문서11개만stage/commit. 보호22+debug.log23개의전후SHA동일,예상밖dirty0,diff--check PASS.
...
- 문서7개f0ca262 commit,code08dc655와함께일반push9429065..f0ca262 성공. HEAD=origin·0/0.
...
### 120 실제 전송 완료 ·119/120 루틴 종료 기록

- 문서7 f30a846 commit,code5d5c30a와일반push276158e..f30a846 성공. HEAD=origin·0/0 확인.
...
## 2026-09-10 — 루틴 재개 · 121 계약 자체검토

- 사용자 `작업이어서 루틴으로 스펙 진행해줘 중요사항아니면 승인 자동처리`에 따라 재개.
- 시작57f6dd0=origin·0/0,staged0. 보호·사용자23 및 번들3 SHA 확보, 보호 변경0.
- 121 신규 코드2/문서7 계약을 먼저 작성·자체검토.119 snapshot→trusted decode→112 admission,
  논리 취소/실제 정착 구분 및 치수/close 결속. 새 제품정책/실사진/UI/운영 허가0.
- 다음 구현·검증 수치는 실측 후 기록. 문서 조회 출력 일부 잘림은 더 작은 범위로 다시 읽었다.

### 121 로컬 구현·검증 완료

- 코드 ec87d65,신규 어댑터/시험2개만. 동일 Codex 자체검수 CODEX_PASSED,독립검수 아님.
- 신규65/targeted166=65+36+42+23 PASS. 전체 check3462=3397+65 PASS(exit0),
  format/lint346·115unit파일·7typecheck·2build. 최초 fake 생성자 lint1건 수정 후 전체 재실행 PASS.
...
### 122 일반 전송 완료 / 다음 실행 지시 보존

- 코드9837a51/문서061af73 일반push 성공,HEAD=origin061af73·ahead/behind0/0 확인.
- 121·122 DONE/CODEX_PASSED(동일Codex),체크/시험 수치는 위실측 그대로. 최종 결과기록은
  같은문서7로1회 전송하고 최종Git을보고한다. 추가영수증커밋/테스트재실행0.
- NEXT 최상단에100 preparation sink·121 lease 결속 계약검토의 읽기순서/정확허용경계/STOP을 남겼다.
...
## 2026-09-11 — 126 전송 / 127 계약·착수

- 126 code91e5b36/docsf3d76e7 일반push,HEAD=originf3d76e7·0/0 확인.
- 127 exactcode4/docs7 계약·자체검토선행통과. 실제100cohort/aggregate안에서명시pairpaint,
  기존모드불변과옛view종료를검증한다. 새pair/native는NOT TESTED,UI/실사진/운영0.
...
## 2026-09-11 — 127 전송 / 128 착수

- 127 d21a49d/033916e 일반push,HEAD=origin033916e·0/0 확인,DONE.
- 128 exactcode6/docs7 계약·자체검토선행. actual102/127/125 합성native60+기존92목표.
  실측전PASS0.기본UI/실사진/운영/보호/자동화0.

## 2026-09-11 — 128 보완 R1

- 최초check typecheck1건:synthetic imageBindings.get의부재반환을기존타입대로undefined로수정.
  수정후check3637/selector38,Chromium20(4.9s) PASS.
- Firefox18/20(52.0s),JPEG/PNG fractional2FAIL. actual/reference RGBAequal:true,
  y=0.75가row0을차지하므로borderClear만false. 잘못된합성좌표를계약/fixture에서1.75로정정.
- 제품코드/단언/게이트완화0. 수정뒤전체check와새3엔진을다시실행한다.
