...
- **`apps/mockup/dist`는 E2E가 아예 쓰지 않는다** → 정상 종료·테스트 실패·시작 실패 어느 경로에서도 fixture 0개이며, 사후 삭제(broad delete)·포트/PID kill·globalTeardown sweep이 **필요 없고 도입하지도 않았다**. 새 서버·포트 0. `.e2e-staging/`은 **gitignored**이며 배포 소스가 아니다(`firebase.json`은 앱 dist를 발행).
