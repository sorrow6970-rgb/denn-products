...
### DONE 보완 (Claude) — 2026-07-28 (Codex "수정 후 재검증" 1건: fixture가 고객 dist 오염)

- **결함 확인(내 이전 주장 정정):** `test:e2e`가 `vite.e2e-fixture.config.ts`(`outDir:"dist"`, `emptyOutDir:false`)를 실행해 **`apps/mockup/dist`에 `e2e-canvas-fixture.html` + 전용 JS/CSS를 append**했다. 따라서 이전 DONE·handoff의 **"production artifact never contains the harness"는 거짓**이었고, `build → test:e2e → deploy` 순서라면 fixture가 배포될 수 있었다. 고객 `/`에 링크가 없다는 검사만으로는 직접 URL 접근·배포 혼입을 막지 못한다는 지적도 타당하다.
- **수정 — E2E 산출물 전량 staging 격리:** fixture config outDir을 **`../../.e2e-staging/mockup`**으로 옮기고, `test:e2e`가 **서빙할 것을 전부 staging에 직접 빌드**한다: `vite build apps/mockup --outDir ../../.e2e-staging/mockup --emptyOutDir` → `vite build apps/admin --outDir ../../.e2e-staging/admin --emptyOutDir` → fixture 빌드(staging에 append) → `playwright test`. `scripts/e2e-preview.mjs`의 `PREVIEW_APPS`가 앱별 `{root, outDir}`을 갖고 Vite `preview()`에 `build.outDir`을 전달해 **staging을 서빙**한다. **스펙 021 exact-handle 소유·teardown·포트 사전 거부·close 경로는 무변경**(서빙 디렉터리만 변경).
- **`apps/mockup/dist`는 E2E가 아예 쓰지 않는다** → 정상 종료·테스트 실패·시작 실패 어느 경로에서도 fixture 0개이며, 사후 삭제(broad delete)·포트/PID kill·globalTeardown sweep이 **필요 없고 도입하지도 않았다**. 새 서버·포트 0. `.e2e-staging/`은 gitignored다. ⚠️ **정정(아래 DONE 보완 2 참조): `firebase.json`은 `hosting.public:"."`(저장소 전체)이고 ignore에 staging 항목이 없으며 `.firebaseignore`도 없다 — gitignore는 배포 제외 근거가 아니므로 저장소 안의 staging은 배포 후보였다. 현재는 OS temp로 옮겼다.**
- **self-contained(§7):** `apps/mockup/dist`·`apps/admin/dist`를 **삭제한 상태**에서 `pnpm run test:e2e`만 실행해도 **57/57 PASS·exit 0**이고, 실행 후에도 **두 dist 디렉터리가 존재하지 않는다** — E2E가 고객 산출물 경로에 아무것도 쓰지 않음을 직접 증명한다.
- **재검증 결과:** clean 상태 고객 build의 **파일 목록+SHA256 기록** → `test:e2e`(57 PASS·summary·exit 0·포트 free·잔류 0) → **고객 dist 목록·해시 IDENTICAL·fixture 관련 파일 0** → **실패 경로 1회**(임시 실패 spec, exit 1) 후에도 **IDENTICAL·0** → 재빌드 시 기록된 해시 재현. frozen exit 0·**lockfile diff 0** / format·lint·typecheck / **unit 468**(467→468: preview 대상이 고객 dist를 서빙하지 않음을 고정하는 테스트 1건) / build(mockup 217.69·gzip 68.40 / CSS 11.32·3.16, admin 193.53·61.09 / 8.54·2.64 — 전부 동일) / check PASS / `git diff --check` clean / 스펙 018 PNG 복원·미커밋.
- **production Canvas surface API·UI 로직은 이번 커밋에서 무변경**(`apps/mockup/src/canvas/**` diff 0). 변경 파일 = `.gitignore`·`package.json`·`scripts/e2e-preview.mjs`·`scripts/e2e-preview.test.mjs`·`apps/mockup/vite.e2e-fixture.config.ts`. 코드/config 커밋 `1f0791d`, 문서 커밋 분리.

### DONE 보완 2 (Claude) — 2026-07-28 (Codex "수정 후 재검증" 2차: staging이 Hosting public 안)

- **결함 확인(내 이전 주장 재정정):** `.e2e-staging/`은 `apps/mockup/dist` 밖이었지만 **Firebase Hosting public 밖이 아니었다**. `firebase.json`의 `hosting.public`은 **`"."`(저장소 전체)**이고 `ignore`에 staging 항목이 없으며 `.firebaseignore`도 없다. firebase-tools는 `**/*`를 `dot:true`로 glob하므로 **gitignore는 배포 제외 근거가 아니다** → 저장소 안의 staging 산출물은 **배포 후보였다**. 이전 문서의 "staging은 배포 소스 아님 / firebase.json은 앱 dist 발행"은 **사실과 다르며** 해당 문장을 모두 정정했다.
- **수정 — staging을 OS temp로 이전:** `scripts/e2e-run.mjs`가 실행마다 **`mkdtemp(os.tmpdir(), "denn-e2e-")`**로 staging을 만들고, mockup·admin·fixture 빌드와 Playwright에 **그 정확한 절대경로**를 전달한다(`DENN_E2E_FIXTURE_OUT_DIR`, `DENN_E2E_STAGING`). `package.json`의 `test:e2e` = `node scripts/e2e-run.mjs`.
- **fail-closed:** `apps/mockup/vite.e2e-fixture.config.ts`는 `DENN_E2E_FIXTURE_OUT_DIR`를 **필수**로 요구하고 **OS temp 밖 경로를 거부**한다 → `dist`나 저장소 경로로 되돌아갈 수 없다. `scripts/e2e-preview.mjs`는 **서빙 디렉터리를 아예 보관하지 않고** spec의 절대 `outDir`을 받으며, 없거나 빈 값이면 서버 기동 전에 거부한다. `tests/global-setup.ts`는 전달받은 staging 경로만 preview하고 **스펙 021 in-process exact-handle 소유·teardown callback·포트 사전 거부·close 경로는 무변경**.
- **cleanup 범위:** 이번 실행이 만든 **정확한 temp 디렉터리 하나만** 제거하며, 가드 `isDisposableStagingPath`가 **OS temp 바로 아래 + `denn-e2e-` 접두사**만 허용한다(temp root 자체·상위·중첩 경로·모든 저장소 경로 거부). **broad delete·포트/PID kill·taskkill·globalTeardown sweep 0**, 새 서버·포트 0. 크래시로 잔여물이 남아도 **저장소·Hosting public 밖**이다.
- **Firebase 설정·Rules 무변경**(§2 준수). `.gitignore`에서 죽은 `.e2e-staging/` 항목만 제거.
