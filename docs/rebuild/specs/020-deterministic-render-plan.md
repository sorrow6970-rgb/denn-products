...
- **안전 경계:** 색상 `#RRGGBB`만(그 외 `INVALID_COLOR`); 안전 식별자(zone.id+imageRef 공통 문법 `^[A-Za-z0-9][A-Za-z0-9._-]*$`·길이 1..128, URI scheme·공백(trim 안 함)·control char·base64 불가→`INVALID_ID`); plan/오류에 URL/base64/token/storagePath·raw catalog 0(직렬화 검사); JSON-safe·성공 plan 전 number finite(`NON_FINITE_RESULT` 안전망); 결정적(Date/random/global 0).
...
- **[2] 안전 식별자 강화:** zone.id·imageRef 공통 문법 **`^[A-Za-z0-9][A-Za-z0-9._-]*$`, 길이 1..128**. URI scheme(`:`)·공백(선행/후행 포함, **trim 안 하고 거부**)·control char·`/`·base64 `+`/`=` 구조적 배제 → URL/base64/token 불가, URL 형태 zone.id가 layerId 미도달. 이전 `trim()`+scheme-prefix 검사(선행 공백 URL 통과)를 대체.
