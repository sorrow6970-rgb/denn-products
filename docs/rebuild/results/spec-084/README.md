...
> **F-3은 스펙 087에서 해소됐다(2026-09-03).** 위 고객 Space 표의 `space-v2-viewer-*.png`와
> `space-v1-blocked-390x844.png`는 canonical 실행이 현재 제품 기준으로 다시 썼다. 인증 후 화면에는 제목이
> 하나(`내 공간 시안` / `이 시안은 지금 화면에 표시할 수 없습니다`)뿐이고 `담당자에게 전달받은 비밀번호를
> 입력하세요.`는 DOM에서 사라졌다. 인증 **전** 화면인 `space-v2-password-gate-390x844.png`는 무변경이다.
