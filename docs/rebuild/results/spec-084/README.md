...
> **F-3은 스펙 087에서 해소됐다(2026-09-03).** 위 고객 Space 표의 V2 열람 화면 두 장과
> V1 차단 화면은 canonical 실행이 현재 제품 기준으로 다시 썼다. 인증 후 화면에는 제목이
> 하나(`내 공간 시안` / `이 시안은 지금 화면에 표시할 수 없습니다`)뿐이고 `담당자에게 전달받은 비밀번호를
> 입력하세요.`는 DOM에서 사라졌다. 위 표의 인증 **전** 비밀번호 입력 화면은 무변경이다.
> 감사 시점 원본은 git history에 남아 있고 다른 finding의 판정은 바뀌지 않았다.

## 캡처하지 않은 매트릭스 항목

**F-4(2026-09-07):** Founder A 승인과 spec090에 따라 위 운영자 기본 화면 두 장을 갱신했다.
데모 카드 3개와 상시 오류는 제거하고 작업 준비 제목·기존 로컬 도구·원격 읽기 상태를 유지했다.
gate off는 그대로이며 새 로그인/저장/발급 경로를 열지 않았다. measurements.json diff 0.
