...
- `packages/render/src/plan/build.test.ts` (신규) — 35 케이스
...
- **imageRef:** opaque·scheme-less 합성 id만. URL 스킴(`data:`/`blob:`/`http:`/`https:`/`javascript:` 등 임의 `scheme:`) 또는 빈/공백 거부 → `INVALID_ID`. **URL/base64/token/storagePath·원본 catalog 객체는 plan/오류에 없음**(직렬화 검사).
...
- **E 오류·누출(로우 다수):** invalid kind(+null) / 케이스 zero·nan canvas·image·scale·pan → 코드 / 액자 zone·color·transform → 코드 / 오류 직렬화에 입력 id marker 0 / imageRef `data:/blob:/http:/https:/javascript:` 거부.

## 9. 전체 게이트

- `corepack pnpm install --frozen-lockfile` exit 0, `pnpm-lock.yaml` diff 0, **신규 의존성 0**.
- `node scripts/check.mjs`: format / lint(`--error-on-warnings`) / typecheck(7) / **unit 332**(스펙 019 297 + plan 35) / build **PASS(exit 0)**.
