...
사용자 `응 승인 다음진행`으로 직전 면책조건 동의·정본 열람 승인. 공식 다운로드267쪽 PDF 확보.
필요한 IFD/Orientation/APP1절 및 방향그림 확인, 상세106 review. 이전 승인 대기 BLOCKED 해소.
106 DOCUMENT_REVIEW_PASSED(동일 Codex), 문서7개만 검증·일반 commit/push 후107 parser-only 계약 작성.
새 제품/test/config변경·실행0. 보호/별도dirty22·실제사진/서비스/설치/운영/예약자동화 금지 유지.

## 최신 — spec106 조사 완료 / 정본 열람 조건 권한 대기 (2026-09-09)

공식 공개 문서와 로컬코드 조사 완료, DOCUMENT_REVIEW_PASSED(동일 Codex 자체 검토, 독립 검수 아님).
CIPA Exif3.1 본문은 면책조건 동의 필요. 수락/다운로드/우회0이며 사용자의 해당 권한 또는 제공 정본 대기.
PNG metadata 유효성과 현재 픽셀 방향 증명을 구분했다. strict/native 및P1/P2는 후보만, NOT TESTED.
105 DONE/CODEX_PASSED·5a4341b 전송 완료.106 제품/test/config변경·실행0, 문서7개만 unstaged.
후속 parser 계약/decoder/UI/실제사진/운영/설치/예약자동화0. STOP에서 stage/commit/push0.
이하 이전 상태는 이력이며 재실행 지시가 아니다. 상세106 review/handoff.

## 최신 — spec105 전송 완료 · 루틴 계속 승인 (2026-09-09)

사용자 `응 큰 문제없다면 별도 승인없이 모든스팩 진행해`는 직전 코드5/문서7 및 동일문서 최종기록 전송 승인과
후속 루틴 계속 지시다. 일반push d063ae1..bc1c240 성공, HEAD=origin·0/0 확인(최종기록 전).
최종 전송기록은 같은7문서에만 남기고 commit/push 후 Git 확인한다. 추가 영수증 반복0.
다음은 배경 방향/EXIF 공식 근거·불변 Blob 소비자 계약 조사. 새 제품 정책/스코프충돌/운영/권한 필요 시 STOP.
예약자동화/실제사진/운영/설치0. 이전 전송 승인 대기는 해소된 이력이다.

## 최신 — spec105 구현·로컬 검증 완료 (2026-09-09)

사용자 루틴 재개 및 도구 복구 후 계속 지시로105 정확5파일 구현, 코드77773db.
targeted83/check unit2924/Chromium292(56.1초), typecheck/format/lint/build PASS. 상세105 계약 DONE/review.
immutable Blob1회 인계·취소/늦은완료·native FileReader 합성 검증, decodeAllowed:false 유지.
고객/admin entry·고객CSS SHA동일. 보호20 SHA동일, PNG2 중 desktop만 canonical 예외로 재생성·복원/커밋0.
코드5+문서7=고유12파일 및 같은7문서의 최종 전송결과 기록을 동일 GitHub 원격/브랜치로 전송할 직접 승인 필요.
원격push는 시도하지 않았다. 문서7개만 별도 로컬커밋 후 최종Git 확인, 나머지dirty22 제외.
다음 방향/EXIF·소비자 계약 조사 후보는 미착수. 실제사진/decoder/UI연결/운영/배포/설치/자동화0.
아래105 미착수/104 완료 포인터는 이전 차례의 이력이다.

## 최신 — 오늘 세션 종료 (2026-09-08)

사용자 `오늘 여기까지,`로 진행 중단. WAITING_FOR_NEXT_MANUAL_TASK.
104 DONE/CODEX_PASSED,105 CONTRACT_REVIEW_PASSED 유지;105 구현·실행은 미착수다.
다음 사용자 재개 지시가 있을 때만 NEXT의105 구현 범위부터 확인한다. 자동 시작/예약자동화0.
이번 종료 기록은 STATE/NEXT/CURRENT/live4개만 갱신한다. 누적105 문서7개 미커밋 유지.
제품/시험/보호파일 변경·stage/commit/push/fetch0. 아래 계약 준비 상태는 기술적 준비 상태이며 세션 재개 승인이 아니다.

## spec105 계약 작성·자체 검토 완료 (2026-09-08)

사용자 `착수해줘`로 NEXT의 bounded file owner 문서 작업을 수행했다.
정본 `docs/rebuild/specs/105-room-background-file-owner-contract.md` 및105 review/handoff.
단발 job·native size-before-read·같은104검사 byte의 불변 Blob 인계·N+N 명시복사·취소/late 경계를 정했다.
READY_FOR_IMPLEMENTATION은 제품 PASS가 아니다. 실제 구현·unit/build/E2E/browser 실행0.
orientation NOT_VERIFIED/decodeAllowed:false,100/102/decoder/제품UI/실제사진/운영/설치0 유지.
현재 문서7개만 미커밋, 보호/별도dirty22 보존. 다음 명시 착수 전 구현0·예약자동화0.
104 DONE/CODEX_PASSED 유지. 아래104 전송/계약 미착수 문구는 각 시점의 완료된 이력이다.

문서 검증 실측: 링크16/16·허용7/7·시작dirty SHA22/22 동일·총dirty29=22+7,
diff--check/신규whitespace PASS·staged0. 기존pnpm-workspace3줄 등 보존, 이번 코드/설정 추가diff0.
HEAD=origin 로컬추적d063ae1·0/0, 원격fetch0. Git global ignore 경고는 설정변경 없이 기록했다.
...
대상 https://github.com/sorrow6970-rgb/denn-products.git 의 rebuild/modern-studio.
전송 후보는 코드6+문서11=고유17파일(차단 영수증5문서도 기존11의 부분집합), 보호/별도dirty22 제외.
사용자 직접 승인 후 현재 HEAD/원격 관계·payload를 다시 확인하고 일반 fast-forward push한다.
BLOCKED는 전송 권한이며 코드검증 실패가 아니다.104 DONE/CODEX_PASSED,96/2841/281 PASS 유지.
새 스펙/계약 착수0. 영수증 커밋 후 정확한 로컬 HEAD/ahead는 Git으로 다시 보고한다.

## 완료 이력 — spec104 (2026-09-08)

최신 사용자 루틴 착수 승인으로 정확6파일 구현 완료, 코드420b2dc. DONE/CODEX_PASSED(동일 Codex).
targeted96=36+36+24, 전체unit2841=2745+96, format/lint315파일/7프로젝트typecheck/build PASS.
canonical Chromium281/281(52.2초), 고객/admin entry·고객CSS SHA불변. 상세104 DONE.
초기 pnpm exec 도구해석 실패는 설치 없이 기존 .CMD 경로로 실행. URL mock/TS 테스트 표기 및
테스트 설명의 Tailwind 후보 단어를 보완했고 최종 게이트를 통과했다. 설정/manifest/lockfile 변경0.
보호·별도20 SHA불변, spec018PNG2만 기존 E2E 재생성 예외·복원/stage/commit0.
포트6개 listener0/이번 staging제거/diff--check PASS. 문서11개 분리 일반commit/push 후 관계 기록.
다음은 bounded file owner 계약 검토 후보.104 밖 구현/실제사진/EXIF/decoder/UI/운영/설치/자동화0.
화면변화0·전체실측진행률 확인불가. 아래 계약만/미착수 기록은 과거 이력이다.

## 계약 검토 이력 — RG-3=A 승인 / spec104 (2026-09-08)

사용자 `승인하고 계약 검증 해줘`로 RG-3=A 승인. 정적JPEG/PNG·20,000,000 bytes·40,000,000 pixels,
초과/미지원/검증불가 거부·자동변환/초과원본축소0. RG-3 결정과104 계약/review/handoff4문서 신규 작성.
103 문서3개의 승인/후속 포인터와 상태4문서를 갱신하여 현재 누적11문서, stage/commit/push0.
104 CONTRACT_REVIEW_PASSED(동일 Codex). 순수 envelope/선언치수 검사만 후속6파일 구현 가능.
성공은 preflight-only/방향 NOT_VERIFIED/decodeAllowed:false. 파일읽기/EXIF/실제decode/취소/UI 연결은 별도.
공개 공식PNG/JPEG/File API 확인. CIPA 최신EXIF 본문 미취득을 명시. 실제사진/서비스 접근0.
102 제품DONE 유지,103 문서DONE·RG-3 CLOSED. 이번 제품/시험/브라우저 실행0, 화면변화0.
전체실측진행률 확인불가. 보호/별도dirty22 보존, 운영/배포/설치/자동화0.
이번은 계약 검증 후 종료하며 다음 명시 구현 착수 지시를 기다린다.
...
## 조사 이력 — spec103 / RG-3 선택 대기 (2026-09-08, 이후 승인으로 해소)

사용자 `다음 작업 이어할까?`로 NEXT 조사를 재개. HEAD=origin 추적cc0bc4c·0/0 확인.
103 DOCUMENT DONE/DOCUMENT_REVIEW_PASSED(동일 Codex), 제품102 DONE/CODEX_PASSED 유지.
상위 성능정본의20MB/40MP 정책을101 누락 근거로 보충. 실제기기 안전성/정확 룸 계약은 별개.
정적 JPEG/PNG·20,000,000 bytes·40,000,000 pixels·초과/미지원 거부 RG-3=A 제안, 미채택.
정확 결과/대안은 docs/codex-claude-handoff/reviews/2026-09-08-spec-103-room-background-input-policy-investigation.md.
문서7개 미커밋, 보호/별도dirty22 유지. 제품/시험/기존증거/브라우저/emulator/실제서비스 접근0.
새 제품 선택 전104 계약·제품·stage/commit/push0. 예약자동화0. 이번 화면변화0·전체실측진행률 확인불가.
문서 검증: 링크/지정라인23/23·시작dirty22/22 SHA동일·허용7경로·diff--check PASS, staged0.

## 종료 이력 — 스펙102 전송 완료 · 오늘 세션 종료 (2026-09-07)
...
## 종료 이력 — spec094 (2026-09-07)

코드·증거11파일38257ef. 제품문구 선택1·신규단위9건·E2E기대문구2개만 변경, 저장 권한/행동 불변.
check2526/2526·canonical267 tests exit0/last-run passed, 변경6PNG 직접 확인, F-9 해소.
출력 절단으로 E2E 말미 시간은 확인 불가; 앱 명령 기록에서 exit0/전체56.626초 재확인, 재실행0.
기존 결과/보호100 중90hash동일, 허용증거8+spec018PNG2예외만 변경. 고객entry불변.
포트6개 잔류0, 이번 staging 부재. 상세 근거는094검수/handoff.
093은1e322c1/cfb1a32/a80cd9c 정상push완료.094코드38257ef/종료5a1abe6도정상push,HEAD=origin5a1abe6·0/0확인.
전송영수증5문서만별도기록한다.기존로드맵/레거시/App/033제외범위읽기전용대조완료,상세근거는094handoff.
다음은 잔여 기능/기존 승인 읽기 전용 대조이며 새 제품 범위·운영 개방은 자동 결정하지 않는다.
전체85~88%는 기존 관리추정 유지, 잔여12~15%=100-추정치. 실측률/최종스펙수 확인불가.

## 종료 이력 — spec093 (2026-09-07)
...
검토이며 최초 timeout 원인 미확정은 유지한다. 다음 후보 F-2 운영자 PNG 선택 표면을 별도 계약으로 다룬다.

**최신 재개 결과:** 사용자가 실패 조사·수정 UI 재검증을 승인했다. 코드/test/config 변경 없이
canonical 단독 1회: **227/227 PASS, 46.3초, skip/retry 0**. 직전 check 2511/2511 PASS 유지.
현재 READY_FOR_CODEX / LOCAL_VERIFIED. 최초 timeout 3건은 미재현이며 원인은 UNCONFIRMED로 보존한다.
사진 선택 PNG 3장과 전체 composer PNG 6장이 최종 생성됐다. 사용자는 spec085 전체 화면과 spec088
확대본으로 시각 검수할 수 있다. 포트 6개 LISTENING 0, staging 제거, 신규 debug.log 없음.
아래 STOP은 이력이다. 새로운 spec/제품 결정/배포/commit/push 없이 종료 검수로 넘긴다.

**현재 STOP:** spec088 최초 E2E 217 PASS / 10 FAIL. UI 관련 7건의 focus/높이를 보완한 뒤 check는
2511/2511 PASS지만 최종 UI E2E는 NOT VERIFIED다. 다른 3건(admin/Space timeout)은 원인 미확정이므로
루프 규칙에 따라 추가 E2E/코드/commit/push를 멈췄다. 정확한 STOP REPORT는 spec088 및 handoff 참조.
보호 20파일 중 기존 canonical이 재생성한 spec018 PNG 2장 외 18개 hash 동일. 포트 6개 LISTENING 0.
보완 전 composer PNG 6장만 존재하며 새 spec088 PNG 3장은 미생성이다. 전체 85~88%는 기존 추정 유지.

최신 정책 정정: 사용자가 **UI도 Codex가 아이디어를 더해 직접 구현**하도록 승인했다. 아래 Claude 전용
인계는 과거 정책이다. 새 스펙 088을 먼저 문서화하고 고객 파일 선택 표면만 구현했으며 검증 차단 중이다.
제품 결정·운영 권한·보호 대상·신규 의존성 금지는 그대로다.

- 사용자 명시 승인 후 `1b0506e`, `a429f0a`, `03cd5e6`을 기존 origin 브랜치로 일반 fast-forward
  push했다. Git 성공 응답과 로컬 원격 추적 ref 대조: HEAD=origin=`03cd5e6`, ahead/behind **0/0**.
  아래 전송 차단은 승인 전의 과거 이력이며 현재 차단이 아니다.
- 승인된 비-UI 범위는 직접 구현·검증하고 다음 안전한 단위를 검토한다. UI 구현·Founder 결정·권한
  확대에서는 멈춘다. 예약 자동화나 백그라운드 반복 작업은 만들지 않았다.
- spec-084 잔여 finding 요청을 대조했다. F-1/F-5/F-3은 085/086/087로 종료, F-6은 철회 상태다.
  다음 후보는 **F-2 파일 선택 UI**이며 고객 표면부터 별도 계약으로 좁힌다. 실제 UI 구현은 Claude 담당.
  F-4 기본 운영자 화면, F-7 진단 노출 정책, F-8 replay 크기 의미는 제품 결정 경계로 보류한다.
  이는 확인한 감사 잔여 항목의 분류이며 리빌드 전체 비-UI 작업 소진을 증명하지 않는다.
- 다음 계약은 아직 작성/승인되지 않았다. NEXT에 계약 작성 시 보존할 동작과 금지 범위를 남겼다.
  현재 상태는 `WAITING_FOR_NEXT_MANUAL_TASK`, next `CODEX_F2_UI_CONTRACT`; 제품 구현 착수 없음.
- 이번 push 이후 갱신한 상태 문서 4개는 미커밋/미스테이지다. 게이트는 재실행하지 않았으며
  아래 2510/2510, 223/223은 직전 스펙 087 재검증 결과다. 진행률은 기존 계획 추정 85~88% 유지.

## 스펙 087 비-UI 보완 직접 수행 · 재검증 통과 · 종료 (2026-09-07)

- **전송 상태 정정:** 로컬 증거 commit `1b0506e`, 문서 commit `a429f0a` 생성 후 push를 요청했으나
  자동 승인 검토가 외부 문서/이미지 전송에 대한 명시 승인 부족을 이유로 거절했다. 원격 동기화는
  완료되지 않았다. 이 정정도 로컬 문서로만 남기며 사용자 승인 전 재시도하지 않는다.
- 사용자 최신 지시: **UI 구현이 아니면 Codex가 직접 진행**. 승인된 비-UI 작업은 Codex가 구현·검증하고,
  실제 UI/UX 구현은 Claude Code 담당이다. 신규 제품 결정·범위 확대·운영 배포 권한을 뜻하지 않는다.
- README F-3 설명의 정확한 파일명 중복 두 곳을 의미 참조로 바꿨다. 출처 표와 테스트는 유지했다.
  이전 `eab7199`의 222/1 실패 이력은 아래에 보존한다.
- 재검증 check exit 0: unit **2510/2510**(92파일), format/lint/typecheck, build 2개 PASS.
  canonical exit 0: **223 passed / 0 failed / 0 skipped / 0 retry**(43.6초). 실패했던 출처 검사도 PASS.
  최종 문서 갱신 뒤 동일 README 유일성/파일 존재 조건도 다시 확인한다.
- 기존 제품 코드를 검수한 Codex가 문서 보완과 재검증까지 수행했다. 별도 에이전트 재검수로 표기하지
  않는다. 판정 **CODEX_PASSED**, 스펙 087 **DONE**. 새 UI/source/CSS/test/config 변경 0.
- PNG 정합 commit `1b0506e`(허용 4장). README + 검수·종료 문서 6개는 별도 문서 commit이다.
  최종 hash는 Git/최종 보고에 남긴다. GitHub 원격은 ls-remote로 확인하며 Firebase 접근은 없다.
- spec-084 PNG 15장과 추가 Space PNG 4장은 검수 때와 SHA-256 동일. build 파일도 동일하다.
  보호 문서/코드 hash 동일, spec-018 PNG는 canonical이 재생성해 desktop `bd5ff207… → d0a0aa52…`,
  mobile `8676d263… → 6bdcb88c…`; restore/stage/commit 0.
- 포트 6개 LISTENING 0, 이번 staging 제거됨. 기존 실패 artifact는 정상 canonical 실행이 정리했고
  ignored `test-results/.last-run.json`만 남았다. check의 chunk-size 경고는 기존대로 존재한다.
- 최종 Git 확인에서 Chromium raster SharedImage 오류 10줄의 `debug.log`(1,210 bytes)를 발견했다.
  E2E 223/223 및 Space PNG hash 동일 근거와 구분해 기록한다. 파일은 삭제하지 않고
  `C:/Users/Public/Documents/ESTsoft/CreatorTemp/denn-spec087-20260907-review/chromium-debug.log`에
  옮겨 보존했다. 브라우저 내부 그래픽 로그가 완전히 깨끗하다고 주장하지 않는다.
- 종료 상태 `WAITING_FOR_NEXT_MANUAL_TASK`, active 없음, next `FOUNDER_NEXT_MANUAL_TASK`.
  전체 리빌드 기존 계획 추정 **85~88% / 잔여 12~15%** 유지. 실제 운영 전환·실기기 검증은 미완이다.

## Codex 스펙 087 독립 검수 — CORRECTION_REQUIRED 라운드 1 (2026-09-07)

- 기준 `HEAD=origin/rebuild/modern-studio=eab7199`, 로컬 원격 추적 ref 대비 ahead/behind 0/0.
  제품 `ac684e3`와 기록 `eab7199`를 함께 검수했다. 이번 원격 fetch는 수행하지 않았다.
- 독립 `node scripts/check.mjs` exit 0: format/lint/typecheck, unit **2510/2510**(92파일), build 2개.
  canonical `node scripts/e2e-run.mjs` exit 1: **222 passed / 1 failed / 0 skipped / 0 retry**.
- 실패는 `local-visual-readiness.spec.ts:933`의 README 출처 유일성 검사다. 기록 커밋 `eab7199`가
  추가한 README F-3 설명에 V1 차단 PNG와 인증 전 PNG의 정확한 백틱 파일명이 각각 재등장해 2회씩
  잡힌다. 제품 커밋 뒤 문서 변경이 원인이며, 이전 223/223 보고를 최종 커밋의 PASS로 사용하지 않는다.
- 제품 신규 E2E 3건 PASS, 제품 추가 결함 미발견. 누락된 `SpaceV2ProofView.tsx`와 테스트(실제 2파일),
  spec-063/spec-080 PNG 4장을 현 스펙의 제목 변경에 필요한 계약 목록 보완으로 인정했다. 상세 범위는 스펙
  및 NEXT의 라운드 1 절을 따른다. PNG는 아직 unstaged다.
- 검수 전후 수집한 보호 파일·Space PNG·build 파일의 SHA-256 차이 0. spec-084 PNG 15장 모두 HEAD와
  diff 0이며 인증 전 gate도 동일하다. 포트 6개 LISTENING 0, 이번 staging 제거 확인. 실패 증거인
  `test-results/local-visual-readiness-eve-47120-one-README-provenance-entry-chromium/error-context.md`와
  `.last-run.json`은 ignored로 남았다(temp 잔류 0과 구분).
- 제품/테스트 직접 수정 및 stage/commit/push 0. 다음 Claude는 README 보완·허용 PNG 정합·전체 게이트
  재검증 후 READY_FOR_CODEX로 돌아온다. 다음 스펙은 시작하지 않는다.
- 전체 리빌드 **85~88% / 잔여 12~15%**는 기존 계획 추정치를 유지한다. 이번 검수로 기능 범위를
  늘리거나 운영 전환을 완료하지 않았으므로 변동 근거가 없다. 정량 완료율 재산정은 하지 않았다.
...
- 기준 `HEAD=origin=5a1aee9`, ahead/behind 0/0. **읽기 전용**이다 — 제품 source·CSS·문구·test·PNG·
  `measurements.json`·config·package/lockfile·Rules 수정 **0**, 게이트 실행 **0**, 새 스펙 작성 **0**,
  스펙 번호 선점 **0**. 실제 Firebase/network/emulator/deploy·실기기 **0**.
- 요청 정본:
...
- F-1은 스펙 085로 닫혔고 남은 후보는 **F-2·F-3·F-4·F-5·F-6·F-7·F-8**이다. 현재 소스 대조 결과
  **F-2·F-3·F-4·F-5·F-7 5건은 유효**하고 **2건은 그대로 스펙화하면 안 된다**.
- **F-6 철회 후보(사실과 다름).** `FramePrintSizeEditor.tsx:104-105`의 root가 이미 `<Card>`이며 spec 041
  (`27e6ff4`) 이후 계속 그랬다. 증거 PNG가 `denn-stack` locator 캡처라 카드 padding/border가 잘렸을 뿐이다.
- **F-8 재도출 필요(제품이 아니라 fixture를 잼).** `space-v2-viewer-*.png`의 320x480은
  ...
  `aspect: 1.5`를 하드코딩한 결과다. 측정 폭 경로(`useContentLogicalWidth` → `resolveFrameLogicalWidth`,
  500 상한)는 이 화면을 그리지 않았고 그 경로의 `composeSpaceFramePlan`은 아직 V1 거절 stub이다
  (`frame-plan.ts:66-84`). 진짜 질문은 **V2 replay가 발행 당시 logical size를 충실 재현해야 하는가**라는
  제품 결정이다.
- **스펙화 난이도.** F-5가 가장 작다 — `admin-space-v2-issue.css:60-85`(spec 083)가 발급 panel의
  `select`/`file`/`range`에 이미 `min-height:44px` + Modern Studio border/radius를 주는 선례다. F-3은 한
  컴포넌트 범위(인증 후 제목 문구 결정 1건 필요). F-2는 두 앱 동시 변경 + spec 026 §5 재선택 계약 보존이
  조건. **F-4는 데모를 지우면 제품 기본값이 빈 화면이 된다**(작업 표면은 gate off) → gate 정책·기본 진입
  화면이 Founder 결정. F-7은 운영 카탈로그 확인이 live network라 현재 금지다.
- **Codex 결정 요청 5건.** ① F-6 철회 여부 ② F-8 재도출·Founder 상신 여부 ③ 다음 단위 범위
  ...
  앱을 함께 넣어도 되는지 ⑤ F-7을 문구 판단만으로 단위화할 수 있는지.
- 상태 `REQUEST_CODEX_NEXT_UNIT_SELECTION`, next `CODEX_NEXT_UNIT_SELECTION`. 보호 대상과 기존
  Founder/user dirty는 수정·restore·checkout·stage·commit **0**. 전체 진행도 **85~88% / 잔여 12~15%**
  (읽기 전용이라 변동 없음).
...
- non-Promise clipboard 반환은 fixed failure로 닫히며 fulfil하는 thenable만 `copied`다. missing port,
  synchronous throw, rejection, non-thenable, hostile `then` getter/call이 모두 안전하게 닫힌다.
- composition과 proof owner는 개발 StrictMode effect replay 동안 하나의 live owner를 유지하고 실제
  unmount에서는 다음 task에 dispose한다. 개발 React fixture가 effect setup 2회를 자기검증하고 baseline,
  PNG decode, Canvas, 단일 issue, observer/listener/URL 수명을 검증한다.
- `apps/admin/vite.e2e-fixture.config.ts` 변경은 스펙 083 대상 파일의 “두 번째 entry가 실제로 필요할 때”
  조건과 일치하며 E2E staging에만 개발 번들을 만든다. 제품/Hosting build 경계는 바뀌지 않는다.
- Codex 독립 `node scripts/check.mjs` PASS(unit **2466/2466**, 92파일, build 2개), canonical Chromium
  **184/184 PASS**. `git diff --check` PASS, 포트·temp 잔류 0, 생성 test 결과/debug 로그 제거.
- 추가 결함 0. 스펙 083은 **CODEX_PASSED / LOCAL_VERIFIED / NO_LIVE_NETWORK**이며 다음은 문서 전용
  closure다. 실제 Firebase/network/emulator/deploy와 다음 스펙은 계속 금지다.

## Claude 스펙 083 보완 라운드 2 (2026-08-31)

- Codex review 문서 대행 commit `4d7f813`, 제품 보완 `1082f55`. 허용 파일 밖 diff 0, package/lockfile/
  Rules/config·고객 앱·기존 spec 064~082 diff 0. `browser-proof-draft.ts`는 무변경.
- 결함 1: `Promise.resolve(write(...))`가 non-Promise 반환을 fulfilled로 만들어 하지 않은 복사를 성공으로
  보고했다 → fulfil하는 thenable만 `copied`, missing/동기 throw/rejection/non-thenable/throw하는 `then`은
  fixed `failed`. 모순된 unit 이름·기대값·문서를 일치시켰다.
- 결함 2: composition(`App.tsx`)과 proof owner(panel)를 mount 동안 살아 있는 record로 전환. cleanup은
  unmounted 표시 후 다음 task에 release하고 StrictMode replay의 두 번째 setup이 취소한다.
- `useLocalImageBinding` replacement 방식은 실측 후 기각: replacement 뒤 stale subtree가 dispose된 write
  controller에 subscribe → auth observer 재부착, idempotent dispose로 detach 불가(2 live / 0 detach).
- 증명은 실제 React 개발 빌드다: fixture config가 같은 entry를 `NODE_ENV=development` define으로 `dev/`에
  한 번 더 빌드하고(신규 dependency 0), fixture는 제품 `useOwnedAdminComposition`으로 composition을
  만든다. E2E가 `fixture-effect-setups === 2`로 자기검증한 뒤 baseline load·PNG decode·Canvas preview·
  단일 issue·auth observer `1:0:1`을 고정한다. 이전 소유권으로 되돌리면 두 E2E 모두 FAIL.
- 실측: `node scripts/check.mjs` PASS(unit **2466/2466**), canonical `node scripts/e2e-run.mjs`
  **Chromium 184 passed / 0 failed**. 라운드 2 flaky spec080 mobile screenshot은 이번 실행 ok(216ms) —
  timeout 증가·skip·retry·고객 코드 수정 0.
- `git diff --check` PASS, 포트 LISTENING 0, temp/`test-results`/`debug.log` 잔류 0. 고객 entry 해시
  무변경, admin entry 295.32 kB(gzip 91.54), lazy `space-write-*.js` 8.47 kB 유지.
- 상태 `READY_FOR_CODEX`, fix_round 2, next `CODEX_SPEC_083_REVIEW_ROUND_3`. 실제 Firebase/network/
  emulator/deploy와 다음 스펙은 계속 금지다.

## Codex 스펙 083 독립 재검수 — CORRECTION_REQUIRED 라운드 2 (2026-08-31)

- 검수 기준 `HEAD=origin=749b2f2`, ahead/behind 0/0. 라운드 1의 clipboard 동기 throw와 post-URL
  `createImage()` throw 보완, auth expiry/unmount/late completion 보완은 적합하다.
- 결함 1: 문서는 non-Promise clipboard 반환을 fixed `copyFailed`로 닫았다고 기록하지만
  `copyLinkToClipboard()`는 `Promise.resolve(undefined)`를 성공으로 바꾸고 unit도 `copied`를 기대한다.
  보고와 코드가 불일치하며 실제 복사 완료 증거가 없는 값을 성공으로 표시한다.
- 결함 2: `App.tsx` composition과 panel proof owner는 ref에 한 번 만든 뒤 effect cleanup에서 dispose한다.
  개발 StrictMode의 effect setup→cleanup→setup에서는 같은 ref가 남아 dispose된 객체가 재사용된다. 현재
  production-build E2E의 실제 unmount→새 mount는 새 component instance/ref를 만들므로 이 경계를 증명하지
  않는다.
- Codex 독립 `node scripts/check.mjs` PASS(unit **2465/2465**, 92파일, build 2개). canonical Chromium은
  spec 083 신규 **21/21 PASS**지만 기존 `space-production-route` spec080 mobile screenshot의
  `preview-canvas` 5초 timeout으로 전체 **181/182**. 원인은 **NOT PROVEN**이며 재시도하지 않았다.
- 자동 루프는 flaky/필수 gate 실패 STOP 조건으로 중지한다. 코드·test 수정, commit/push 0. 생성된
  `test-results`/`debug.log`만 안전하게 제거했고 포트·temp 잔류 0이다.
- 상태 `CORRECTION_REQUIRED`, fix_round 2, next `CLAUDE_SPEC_083_CORRECTION_ROUND_2`. 실제 Firebase/
  network/emulator/deploy와 다음 스펙은 계속 금지다.

## Claude 스펙 083 보완 라운드 1 (2026-08-31)

- Codex review 문서 대행 commit `1d03bfc`, 제품 보완 `7ce9ab4`. 허용 파일 밖 diff 0, package/lockfile/
  Rules/config diff 0, `App.tsx` 무변경.
- 결함 1: copy 결정을 `copyLinkToClipboard()`로 분리 — missing port · 동기 throw · rejection ·
  non-Promise 반환 모두 fixed `copyFailed`로 닫는다. success·link 보존, raw error 미노출.
- 결함 2: object URL 생성을 자기 단계로 분리 — 이후 어떤 실패도 정확히 1회 revoke를 지난다.
- 추가 결함(보완 중 발견): 발급 중 auth 만료가 definite auth 실패와 outcome-unknown 경고를 baseline
  문구로 덮었다. 이미 일어난 시도를 먼저 보고하도록 status 순서만 바꿨다.
- 신규 검증 3건은 **수정 전 소스에서 FAIL**을 직접 확인했다. unit +7(2465/2465), spec 083 E2E 16 → 21.
...
- `git diff --check` PASS, 포트 LISTENING 0, temp/`test-results`/`debug.log` 잔류 0. 고객 entry 해시
  무변경, admin entry 294.80 kB(gzip 91.35), lazy `space-write-*.js` 8.47 kB 유지.
- StrictMode dev 빌드 관찰(`App.tsx` compositionRef / panel ownerRef 재생성 없음)은 기록만 하고 고치지
  않았다 — Codex 판단 요청.
...
- 기준 `HEAD=origin=60507b3`, ahead/behind 0/0. Codex 재검수·NN-2 문서 commit `60507b3`, 보완 commit
  `65c5b46`. Founder **NN-2=A**가 허용한 **제품 파일 한 개**
  (`tests/e2e/admin-auth-read.spec.ts`)만 고쳤고 제품 코드는 한 줄도 바꾸지 않았다.
- **전체 Chromium E2E가 160 passed / 0 failed다**(기존 159 + 신규 call-surface 테스트 1). 라운드 1까지
  남아 있던 158/1이 해소됐다.
- **결함.** `bundle.includes("uploadBytes")`는 번들된 Firebase 제품이 모듈 전체를 싣는 탓에 "이 앱이
  쓰기를 하는가"가 아니라 "Storage SDK가 존재하는가"를 측정했다. 그 이름들은 vendor chunk의 export
  맵과 `_throwIfRoot("uploadBytes")` 오류 라벨에 있을 뿐 고객 호출부가 아니다. 같은 목록이
  `getStorage`도 금지했는데 그건 스펙 079(MM-1=A)가 승인한 호출이라, 079 이후 이 검사는 통과할 수
  없는 상태였다.
- **보완.** 번들 substring 목록에서 Storage 이름 6개를 빼고 vendor가 만들지 않는 app-level 문자열만
  남겼다(`admin-read`·`ADMIN_STATE_OBJECT_PATH`·`admin/state.json`·`onAuthStateChanged`·
  `signInWithEmailAndPassword`). 라운드 1의 `getAuth` whole-identifier 검사, positive marker,
  default route external request 0 검사는 그대로 유지했다. 대신 신규 테스트가 고객의 **자기 소유
  production source**(`apps/mockup/src` + 고객이 실제 import하는 유일한 subpath
  `packages/firebase/src/space-read`, unit test·`e2e/` 제외, 총 58파일)를 주석 제거 후 검사한다 —
  ① write/admin subpath import **0**(이 단언이 package 절반을 `space-read`로 한정한 근거이자, 고객이
  다른 subpath에 손대면 먼저 깨지는 가드) ② `uploadBytes`·`uploadBytesResumable`·`uploadString`·
  `updateMetadata`·`deleteObject`·`list`·`listAll`·`getDownloadURL`·`getBlob`·`getStream` **호출 0**
  ③ `getStorage`·`ref`·`getMetadata`·`getBytes` 호출이 **실제로 존재**(없으면 호출이 스캔 밖으로
  옮겨가도 통과해버린다).
- **검사에 이빨이 있다는 측정 증거.** 같은 검사를 admin write surface에 겨누면 `uploadBytes`가 **FAIL**로
  잡힌다. Storage vendor chunk의 `.이름(` 호출 형태는 **0**이라 export 맵이 호출부로 위장할 수 없고,
  고객 app chunk는 허용 4종만(`getStorage` 1·`ref` 2·`getMetadata` 1·`getBytes` 1) 쓰고 금지 0이다.
  호출 형태를 보므로 vendor 문자열에 오염되지 않고, source를 보므로 bundler chunk 분할에 의존하지 않는다.
- **실측.** targeted `admin-auth-read` Chromium **4/4**, **전체 Chromium E2E 160 passed / 0 failed**,
  전체 `node scripts/check.mjs` **PASS**(unit **2409/2409**), **build 산출물 14개 모두 보완 전과
  byte+SHA-256 동일**(제품 코드 무변경), `git diff --check` PASS, 변경 경로는 허용 한 파일뿐,
  EOL `i/lf w/lf`, 검사 포트 6개와 `test-results`/temp 잔류 **0**.
- **약화 없음.** 테스트를 삭제하지 않았고 E2E 예외로 처리하지도 않았다. Storage 쓰기·열거·다운로드
  경계는 오히려 **더 강해졌다** — 예전엔 vendor 문자열 유무만 봤지만 이제 실제 호출과 import 경계를
  본다. 스펙 079/080의 read-only Storage 승인은 보존했다.
...
- 전체 진행도 **84~87% 완료 / 13~16% 잔여 — 변동 없음**(검증 정확도 보완이며 새 제품 능력이 아니다).
