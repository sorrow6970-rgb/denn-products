...
completed_unit: spec-087-space-post-auth-header-collapse   # DONE, CODEX_PASSED, LOCAL_VERIFIED, NO_LIVE_NETWORK
active_unit: none
...
verified_commit: ac684e3   # product reviewed; README correction revalidated, PNG reconciliation 1b0506e
origin_relation: "LOCAL_COMMITS_ONLY; push blocked by automatic approval review; origin remains eab7199; local HEAD is ahead (final count in report)"
working_tree: "after closure commit only pre-existing protected/user dirty remains; spec-018 PNGs regenerated by canonical E2E stay unstaged"
fix_round: 1
max_fix_rounds: 3
next_transition: FOUNDER_NEXT_MANUAL_TASK
automation_loop: stopped (no scheduled automation; approved non-UI work by Codex, UI implementation by Claude Code)
session_status: spec 087 DONE locally, CODEX_PASSED; push requires explicit destination/payload approval after automatic approval rejection; no next spec started
commit_owner: Codex for this non-UI correction, verification, evidence and closure commits per 2026-09-07 user instruction
```
